...
abstract contract Test_BalancerLPGateReinvestBase is Test_BalancerSwapRateOracleBase {
    ...
    /**
     * @notice DIVERGENCE 10 — the pool's LP set is PERMISSIONLESS and the liquidity gate is blind to venue
     ...
     */
    function test_DIVERGENCE_10_poolPermissionless_externalExitDrainsDepthGateBlind() public {
        _seedForSwaps();
        _sync();
        ...
        assertLe(depthAfter, Math.mulDiv(depthBefore, 55, 100), "DIVERGENCE: half the real tradable depth left the venue");

        _sync();
        assertLe(_committedLiquidityUtilization(), utilBefore + 1, "DIVERGENCE: the liquidity utilization did not move (the gate is depth-blind)");
    ...
    /**
     * @notice DIVERGENCE 10, companion in the inverse direction — EXTERNAL depth cannot release a binding
     ...
     */
    function test_DIVERGENCE_10b_externalDepthCannotReleaseGate() public {
        _seedForSwaps();
        _arrangeYieldDrivenLiquidityBreach();
        ...
        assertGt(_committedLiquidityUtilization(), WAD, "DIVERGENCE: doubling the REAL depth does not move the committed utilization");
        uint256 shares = LT.balanceOf(LT_ALICE_ADDRESS) / 10;
        vm.prank(LT_ALICE_ADDRESS);
        vm.expectRevert(IRoycoDayAccountant.LIQUIDITY_REQUIREMENT_VIOLATED.selector);
        LT.redeem(shares, LT_ALICE_ADDRESS, LT_ALICE_ADDRESS);
