...
import { BalancerSwapRateOracleForkSuite } from "./BalancerSwapRateOracleForkSuite.sol";

/**
 * @title BalancerLPGateReinvestForkSuite
 * @notice Fork batteries D (external LP through the canonical Router), E (the liquidity gate on real oracle
 *         numbers), F (reinvestment fee decomposition on real E-CLP math), G (proportional-remove composition
 *         after skew), and H (FIXED_TERM x the pool). Chains linearly on the swap/rate/oracle suite so one
 *         concrete leaf carries the whole deep-venue battery.
 */
abstract contract BalancerLPGateReinvestForkSuite is BalancerSwapRateOracleForkSuite {
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    function _committedLiqUtil() internal view returns (uint256) {
        IRoycoDayAccountant.RoycoDayAccountantState memory a = ACCOUNTANT.getState();
        return _expectedLiqUtil(a.lastSTEffectiveNAV, a.minLiquidityWAD, a.lastLTRawNAV);
    }
    ...
    function _arrangeYieldDrivenLiquidityBreach() internal {
        _sync();
        _enableLTOverlay(0.1e18, 0, _minLiquidityForTargetUtil(0.99e18));
        for (uint256 i = 0; i < 20; ++i) {
            _warpForward(1 days);
            _applySTYield(0.02e18);
            _sync();
            if (_committedLiqUtil() > WAD) return;
        }
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // D — EXTERNAL LP THROUGH THE CANONICAL ROUTER
    // ═══════════════════════════════════════════════════════════════════════════

    /// @notice D1 — an external unbalanced add syncs the kernel through the hook exactly once and never
    ...
    /**
     * @notice D2 — an external SINGLE-SIDED add's realized cost matches the derived leak law
    ...
    /// @notice D3 — an external PROPORTIONAL add is value-neutral to everyone else: NAV-per-BPT and the
    ...
    /**
     * @notice D4 / FINDING 10 — the pool's LP set is PERMISSIONLESS and the liquidity gate is blind to venue
    ...
    function test_FINDING_10_poolPermissionless_externalExitDrainsDepthGateBlind() public {
        ...
        _driveLiqUtilTo(0.95e18);

        uint256 utilBefore = _committedLiqUtil();
        ...
        _sync();
        assertLe(_committedLiqUtil(), utilBefore + 1, "FINDING: the liquidity utilization did not move (the gate is depth-blind)");
    ...
    /**
     * @notice D5 — an external single-token exit pays the imbalance fee to the remaining LPs: NAV-per-BPT
    ...
    /**
     * @notice D6 — external depth is economically inert to a kernel redeemer: a multi-asset LT redemption pays
    ...
    /// @notice D7 — a paused hook blocks BOTH external add and external remove (the `router != kernel` path of
    ///         each before-hook syncs `whenNotPaused`), completing the pause blast-radius picture of battery A.
    ...
    function test_ExternalAddRemove_hookPaused_reverts() public {
        _seedForSwaps();
        _sync();
    ...
    /// @notice D8 — Balancer's unbalanced-add invariant-ratio cap (5x) is the hard bound behind the seed
    ///         helpers' 3x chunk rule: a single external add of ~6x the pool's value reverts.
    function test_ExternalAddUnbalanced_invariantRatioCap_reverts() public {
        _seedForSwaps();
        _sync();
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // E — THE LIQUIDITY GATE ON REAL ORACLE NUMBERS
    // ═══════════════════════════════════════════════════════════════════════════

    /**
     * @notice E1 — the committed liquidity utilization equals the independent ceil-mirror
     *         `ceil(stEff * minLiq / ltRaw)` on REAL oracle inputs, across seeded, skewed, externally-deepened,
    ...
    function _assertPacketUtilMatchesMirror(string memory _ctx) internal {
        ...
        assertEq(
            ...
            _expectedLiqUtil(s.stEffectiveNAV, uint64(s.minLiquidityWAD), s.ltRawNAV),
    ...
    /**
     * @notice E2 — the liquidity gate binds senior entry at exactly WAD on real numbers and is RELEASED by real
    ...
    function test_LiquidityGate_stDeposit_bindsAtWAD_releasedByLTDeposit() public {
        _seedForSwaps();
        _driveLiqUtilTo(0.99e18);

        // Headroom from the committed checkpoint: util <= WAD  <=>  stEff <= ltRaw * WAD / minLiq.
        IRoycoDayAccountant.RoycoDayAccountantState memory a = ACCOUNTANT.getState();
        ...
        assertLe(_committedLiqUtil(), WAD, "the post-deposit committed utilization must respect the gate");
    }

    /**
     * @notice E4 — a purely yield-driven breach (no one acted) blocks depth-reducing LT redemptions and senior
    ...
    function test_LiquidityGate_yieldDrivenBreach_blocksLTRedeemAndSTDeposit() public {
        ...
        _seedLTBalanced(LT_BOB_ADDRESS, _rawBalances()[_stPoolIndex()] / 4);
        assertLe(_committedLiqUtil(), WAD, "arrange: the deepening must heal the utilization");
        vm.prank(LT_ALICE_ADDRESS);
        LT.redeem(shares, LT_ALICE_ADDRESS, LT_ALICE_ADDRESS);
    }

    // ═══════════════════════════════════════════════════════════════════════════
    // F — REINVESTMENT FEE DECOMPOSITION ON REAL E-CLP MATH
    // ═══════════════════════════════════════════════════════════════════════════

    /// @dev The staged idle premium's NAV value at the committed senior mark (the venue's own reinvest basis).
    function _idlePremiumValueNAV() internal view returns (uint256 idleShares, uint256 idleValueNAV) {
    ...
    function _probeReinvestHaircutWAD() internal returns (uint256 haircutWAD, uint256 fairBPT) {
        ...
        (, uint256 idleValueNAV) = _idlePremiumValueNAV();
    ...
    /**
     * @notice F1 — reinvest wealth conservation: deploying the staged premium moves EXACTLY its value into the
    ...
    function test_Reinvest_wealthConservation_identity() public {
        _arrangeReinvestableIdlePremium();
    ...
    /**
     * @notice F2 — the reinvest obeys the single-sided leak law through the PRODUCTION path: the value the LT
    ...
    /**
     * @notice F3 — the slippage gate flips at exactly the measured real-math haircut h*: a threshold below h*
    ...
    function test_Reinvest_gateFlipsAtMeasuredHaircut() public {
        ...
        (uint256 idleShares0,) = _idlePremiumValueNAV();
    ...
    /**
     * @notice F4 — the SHIPPED slippage default (the market config's 10bp) clears the realized haircut on a
    ...
    /**
     * @notice F5 — the leak law holds across pool composition states: the reinvest's realized leak matches
    ...
    /**
     * @notice F6 — the venue's REAL failure mode is tolerated: a premium too large for the pool's unbalanced-add
     *         invariant-ratio cap (a dust-deep pool, the `_arrangeStagedIdlePremium` state) reverts inside
     ...
     *      protection there — battery F5 exercises exactly that); (2) the kernel's ANY-revert-swallowed
    ...
    function test_Reinvest_invariantRatioCapFailure_tolerated_sharesStayIdle() public {
        _arrangeStagedIdlePremium(); // shallow pool with a staged premium (gate shut during staging)
        ...
        for (uint256 i = 0; i < 10; ++i) {
            (, idleValueNAV) = _idlePremiumValueNAV();
        ...
        uint256 idleShares0;
        (idleShares0, idleValueNAV) = _idlePremiumValueNAV();
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // G — PROPORTIONAL-REMOVE COMPOSITION AFTER SKEW
    // ═══════════════════════════════════════════════════════════════════════════

    /**
     * @notice G1 — after swaps skew the reserves, a kernel multi-asset redemption removes both venue legs in
    ...
    /**
     * @notice G2 — after a skew, a multi-asset redeemer is never OVERCHARGED and inherits at most its slice of
    ...
    /// @notice G3 — Router-path control for G1's formula: an EXTERNAL proportional remove returns exactly the
    ...
    /**
     * @notice G4 — a value-balanced multi-asset deposit into a SKEWED pool pays a bounded entry cost: the
    ...
    /**
     * @notice G5 — the in-kind BPT slice is COMPOSITION-INDEPENDENT: an identical in-kind redemption pays the
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // H — FIXED_TERM x THE POOL
    // ═══════════════════════════════════════════════════════════════════════════

    /// @notice H1 — the pool is NOT frozen by a fixed term: external swaps execute in-term, the hook still
    ...
    /// @notice H2 — external LP add and remove both function in-term: the venue's LP surface is term-agnostic.
    ...
    /**
     * @notice H3 — the quote-only multi-asset deposit (the ONLY kernel deposit legal in-term) lands as a real
    ...
    /**
     * @notice H4 — in-term the premium machinery is silent but the STAGED premium is not stranded: an in-term
    ...
    /**
     * @notice E5 / FINDING 10 (companion of D4, inverse direction) — EXTERNAL depth cannot release a binding
    ...
    function test_FINDING_10b_externalDepthCannotReleaseGate() public {
        _seedForSwaps();
        _arrangeYieldDrivenLiquidityBreach();
        assertGt(_committedLiqUtil(), WAD, "arrange: the gate must be binding");

        _externalProportionalPosition("EXTERNAL_RESCUER_LP", _bptSupply()); // doubles the real venue depth
        _sync();

        assertGt(_committedLiqUtil(), WAD, "FINDING: doubling the REAL depth does not move the committed utilization");
        uint256 shares = LT.balanceOf(LT_ALICE_ADDRESS) / 10;
        vm.prank(LT_ALICE_ADDRESS);
        vm.expectRevert(IRoycoDayAccountant.LIQUIDITY_REQUIREMENT_VIOLATED.selector);
        LT.redeem(shares, LT_ALICE_ADDRESS, LT_ALICE_ADDRESS);
