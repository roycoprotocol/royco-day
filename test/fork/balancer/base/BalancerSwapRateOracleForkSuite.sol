...
/**
 * @title BalancerSwapRateOracleForkSuite
 * @notice Fork batteries A (real E-CLP swaps + hook coupling), B (getRate / rate provider), and C (computeTVL
 *         on the seeded pool) — the real-Balancer-math surfaces the mock layers cannot reproduce
 *         (testing-strategy fork plan). Runs on the market the concrete leaf configures.
 * @dev Cache regime notes per test follow the discipline documented on `BalancerVenueForkBase`.
 */
abstract contract BalancerSwapRateOracleForkSuite is BalancerVenueForkBase {
    // ═══════════════════════════════════════════════════════════════════════════
    // SHARED ARRANGE HELPERS
    // ═══════════════════════════════════════════════════════════════════════════

    /// @dev Standard swap-battery arrange: seeded ST/JT market plus a default-depth real pool.
    ...
    /**
     * @notice A1 — an external swap's before-hook syncs the kernel FIRST: the committed checkpoint equals what
    ...
    /**
     * @notice A3 — swap fees accrue to the BPT: the oracle TVL rises by exactly the pool-retained fee, the BPT
    ...
    /**
     * @notice A4 — the kernel's LT raw NAV captures exactly its pro-rata share of the swap-fee TVL growth:
    ...
    /**
     * @notice A5 — a paused hook blocks external swaps (the before-swap sync is `whenNotPaused` and its revert
     *         bubbles), and unpausing restores swap liveness. Pins the liveness coupling of third-party pool
     *         flow to Royco pause state.
     */
    function test_ExternalSwap_hookPaused_reverts_thenRecovers() public {
        _seedForSwaps();
        (address swapper, uint256 amountIn) = _armSwapper(testConfig.quoteAsset, 0.25e18);
    ...
    /**
     * @notice A6 — a paused hook does NOT touch kernel-routed LT flows: the `router == kernel` carve-out
    ...
    /**
     * @notice A7 — the E-CLP price range is a hard wall: the probed boundary capacity executes, ten percent
     *         beyond it reverts `AssetBoundsExceeded` from the Gyro math.
     */
    function test_ExternalSwap_beyondRangeBoundary_reverts_AssetBoundsExceeded() public {
        _seedForSwaps();
    ...
    /**
     * @notice A8 — swap capacity matches the band geometry: at a range boundary the pool holds a single asset,
     *         so each direction's value capacity approximates the OPPOSING leg's live balance. Documents the
     *         sizing rule every skew-driven battery relies on.
    ...
    /**
     * @notice A9 / FINDING 8 — swaps are NOT blocked in the block of a P&L sync. CLAUDE.md ("Slippage gate and
    ...
    /**
     * @notice A10 / FINDING 9 — the rate-staleness LVR arb is impossible THROUGH THE POOL. CLAUDE.md
    ...
    /**
     * @notice A11 — a round trip cannot profit and pays at least (approximately) two fee legs: no-free-lunch
    ...
    /**
     * @notice A2 — an executed swap's realized price stays inside the fee-adjusted E-CLP band, both directions.
    ...
    /**
     * @notice B1 — `getRate` equals the committed senior NAV per share: `floor(WAD * lastSTEffectiveNAV /
     ...
     *      (Cache-miss vs live-path parity is pinned in the mock battery `test/unit/balancer/BptOracleQuoter`;
    ...
    /// @notice B2 — within a synced transaction the rate is FROZEN: a feed move after the sync does not move
    ...
    /**
     * @notice B3 — the rate is monotone under senior yield and, after a premium-minting sync, still equals the
    ...
    /**
     * @notice B4 — an external swap refreshes the rate through the hook: after a feed move, the swap leaves the
    ...
    /// @notice B5 — the Vault's own view of the pool token rates reads the kernel rate provider live: the
    ...
    /**
     * @notice C1 — on the seeded real pool the oracle TVL is a nonzero value inside the rate-scaled balance
    ...
    /**
     * @notice C2 — manipulation resistance, quantified: a swap of half the pool's capacity moves the oracle TVL
    ...
    /// @notice C3 — the TVL/MtM band survives a near-boundary skew: even at maximal in-range composition
    ...
    /**
     * @notice C4 — the oracle's senior leg composes the kernel's `getRate`, not its constant-1.0 feed: a feed
    ...
    /**
     * @notice C5 — `computeTVL` reads cleanly INSIDE `Vault.unlock`: an external Router add triggers the hook's
    ...
    /// @notice C6 — the kernel's LT conversions round-trip on the LIVE oracle TVL with bounded floor loss:
