...
abstract contract Test_BalancerSwapRateOracleBase is BalancerVenueForkBase {
    ...
    function test_GetRate_matchesCommittedSeniorNAVPerShare() public {
        ...
        // shares/assets offset: floor((stEff + VIRTUAL_ASSETS) * WAD / (supply + VIRTUAL_SHARES))
        assertEq(
            rate,
            Math.mulDiv(WAD, stEff + VIRTUAL_ASSETS, supply + VIRTUAL_SHARES),
        ...
        assertLe(rate * (supply + VIRTUAL_SHARES), WAD * (stEff + VIRTUAL_ASSETS), "rate * effective supply must never overstate the committed senior NAV");
        assertGt(
            rate * (supply + VIRTUAL_SHARES) + (supply + VIRTUAL_SHARES),
            WAD * (stEff + VIRTUAL_ASSETS),
    ...
    function test_GetRate_monotoneUnderYield_tracksCommittedMark() public {
        ...
        // getRate carries the offset: floor((stEff + VIRTUAL_ASSETS) * WAD / (postMintSupply + VIRTUAL_SHARES))
        uint256 expected = Math.mulDiv(WAD, toUint256(ACCOUNTANT.getState().lastSTEffectiveNAV) + VIRTUAL_ASSETS, ST.totalSupply() + VIRTUAL_SHARES);
    ...
    function test_GetRate_externalSwapRefreshesRateThroughHook() public {
        ...
        // getRate carries the offset: floor((stEff + VIRTUAL_ASSETS) * WAD / (supply + VIRTUAL_SHARES))
        uint256 expected = Math.mulDiv(WAD, toUint256(ACCOUNTANT.getState().lastSTEffectiveNAV) + VIRTUAL_ASSETS, ST.totalSupply() + VIRTUAL_SHARES);
