...
import { Identical_ERC4626_ST_JT_SharePriceToChainlinkOracle_BalancerV3_BPTOracle_LT_Kernel as DayKernel } from
 ...
 *      impossible at the kernel layer and independent (stRaw, jtRaw) tuples are driven at the accountant harness
 */
abstract contract TrancheFixture is Assertions {
    ...
    /// @notice The token-matrix cell this market was deployed with
    ...
    /**
     * @notice Deploys the full Day market for the specified cell and parameterization
     * @dev Mirrors BalancerV3DeploymentTemplate.deployMarket's order, adapted to mocks: tokens, oracles, venue, YDMs,
     ...
     * @param _cell The token-matrix cell to deploy
    ...
    function _deployMarket(FixtureCell memory _cell, MarketParamsConfig memory _params) internal virtual {
        ...
        // construction (RoycoDayKernel.sol:122). The jtCoinvested=false axis lives only in the accountant harness
        require(_params.jtCoinvested, "TrancheFixture: kernel family forces jtCoinvested=true; drive the false axis at the accountant harness");
        _validateCell(_cell);
        ...
        // proxies, so ST lands at index 1 and the quoter constructor's tokens[1] == SENIOR_TRANCHE branch
        // (BalancerV3_LT_BPTOracle_Quoter.sol:103) is exercised by the whole smoke battery, not forced artificially
        require(
            address(balancerVault.getPoolTokens(address(bpt))[stPoolTokenIndex]) == address(seniorTranche),
            "TrancheFixture: recorded senior pool index does not match the registered token order"
        );
        ...
        require(kernelProxy == predictedKernel, "TrancheFixture: kernel proxy address prediction failed");
     ...
     *      independent (stRaw, jtRaw) tuples are driven at the accountant-harness layer
    ...
    function applyQuotePnL(int256 _bps) internal virtual {
        ...
        require(factorWAD > 0, "TrancheFixture: quote PnL factor must stay positive");
    ...
    function syncVenuePrices() internal virtual {
        ...
        (, int256 answer,,,) = priceFeed.latestRoundData();
        require(answer > 0, "TrancheFixture: cannot sync venue prices off a non-positive feed answer");
    ...
    function setOracleMode(uint8 _mode) internal virtual {
        if (_mode == ORACLE_MODE_NONE) {
            ...
            revert("TrancheFixture: unknown oracle mode");
        }
    }
    ...
    function _acquireSTShares(uint256 _shares) internal virtual {
        ...
        require(seniorTranche.balanceOf(address(this)) - balanceBefore >= _shares, "TrancheFixture: ST share acquisition undershoot");
    }
    ...
    /// @notice Validates the cell satisfies the kernel family's structural constraints
    function _validateCell(FixtureCell memory _cell) internal pure {
        require(_cell.stAsset.erc4626 && _cell.jtAsset.erc4626, "TrancheFixture: ST/JT assets must be ERC4626 cells for this kernel family");
        require(!_cell.quoteAsset.erc4626, "TrancheFixture: the quote asset must be a plain ERC20");
        require(
            ...
            "TrancheFixture: ST and JT token configs must be identical (one shared vault instance)"
        );
    }

    /// @notice Deploys a configurable ERC20 and applies the cell's behavior bitmap and fee
        ...
        returns (IYDM ydm, bytes memory initData)
    {
        if (_kind == 0) {
            ...
            revert("TrancheFixture: unknown YDM kind");
        }
    ...
    // =============================
    // Role Wiring (mirrors BalancerV3DeploymentTemplate._buildRoleBindings minus Balancer-governance targets)
    // =============================
    ...
    function _wireTargetFunctionRoles() internal {
        ...
        // Accountant (the template's 16-selector list, BalancerV3DeploymentTemplate.sol:525-561)
