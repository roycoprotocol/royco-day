...
> regenerated by `Test_BandWidthCandidates.t.sol` and `Test_C1FullBattery.t.sol` — and **§7 is
...
shipping values differ on exactly the two knobs the band-width studies revisited — **α = 1/1.015
...
C1 band replaces the production −15 bp floor). Sections 1–6 measure the first decision on the
original A/D candidates; this section carries the final word on both. Every C1 number below is
measured on the real vault by `Test_BandWidthCandidates.t.sol` (T8) and `Test_C1FullBattery.t.sol`
(T9, 12/12 passing) — same standard as the rest of this document.*

### 7.1 The parameters

| Knob | Value (18-dec on-chain literal) | Why this value |
|---|---|---|
| **α** (band floor) | `985_221_674_876_847_291` = 1/1.015 | The pool keeps quoting exits down to ~148 bp under marked NAV, and that discount is the **restock carrot**: at 95% drained the pool quotes −115 bp and an arber refilling it nets 38 bp per dollar of stables returned. The old −15 bp floor paid 2.9 bp — nobody's hurdle |
| **λ** (concentration) | `300_000_000_000_000_000_000` (= 300) | How *early* the carrot ramps. At λ=300 the discount crosses 10 bp by 16% drained and 25 bp by 42% — arbs wake while the pool is still mostly full. At the old λ=4000, the restock arb could not even fire (discount < fee) until ~20% drained. λ also sets depth: a $1M clip on $10M TVL still costs only ~4 bp all-in |
| **β** (upper bound) | `1_000_000_607_199_568_608` | Not a free choice — solved (100-digit pipeline) so the peg sits exactly at the 99.99% stable corner. β−1 = 0.0061 bp is the entire premium a pinned pool ever quotes |
| **Rotation c, s** | `707_106_781_186_547_524` (both) | 45°: maximum depth exactly at price 1.0, decaying monotonically toward α (measured: 12.5% of peak density at D95) |
| **Fee** | 1 bp (`1e14`) | Must satisfy the one non-negotiable invariant **fee > β−1** (margin here: 165×). Do not raise it to chase LP income — the fee is the arb *hurdle*, and arbs are the restoration mechanism this whole design buys |
...
**Derived params** (38-dec `DerivedEclpParams`): the `_derivedParamsC1()` literals in
`Test_C1FullBattery.t.sol`, produced by the mpmath pipeline after it reproduced every
...
on-chain. **Balance-ratio seed**, if used: `1_000_100_010_000_524_809_194` wei of ST per $10M
of stables.
...
1. **Cross the hurdle while the pool is still mostly full.** C1: 10 bp at 16% drained, 25 bp at
   42%. (Old band: the arb cannot fire below ~20% drained and never pays more than ~10 bp.)
2. **Peak before exhaustion, with stress headroom.** C1: 115 bp at 95% drained, rising to ~148 at
   the floor — 4–10× cover over the hurdle exactly when it matters. (C2's 100 bp cap flattens the
   signal precisely when it should steepen; that killed it.)
3. **Keep routine clips cheap.** C1: ~4 bp all-in for a $1M exit at balance — excellent for an
...
margin at 95% drained): **A (old band) 10.4 → 2.9 bp · C1 115 → 38 bp · C2 (α=1/1.01, λ=300)
85 → 32 bp · C3 (α=1/1.015, λ=500) 92 → 26 bp.** On the $10M reference pool, refilling a
95%-drained C1 pays the arber **$35.8k per event** (and restores 98% of the drain); the old band
paid $2.1k.
...
| Drained | Exiter's volume-weighted haircut (C1) | Old band | Spot discount quoted (C1) |
|---|---|---|---|
| 10% | 4.0 bp | 1.2 bp | 6.1 bp |
| 25% | 8.7 bp | 1.6 bp | 15.6 bp |
| 50% | 17.1 bp | 2.3 bp | 34.1 bp |
| 95% | 39.5 bp | 4.1 bp | 115.2 bp |

Drained-state LP flows likewise carry real impact now: one-way single-sided adds up to ~3 bp at
...
### 7.5 What T9 re-verified (nothing shield-gated moved)
...
haircut at 33.79 bp — both properties of the tilt and the marking calendar, not the band.
Flow-breakeven grid: all 9 points classify identically to A (same 0.73-day breakeven);
sub-breakeven leak is impact crumbs only (0.044 bp/yr — larger than A's 0.0035 by design,
still noise).
...
- **If real-world arb hurdles measure higher than ~40 bp** (e.g. redemption queues make ST
  warehousing expensive in stress): deepen α toward 1/1.02 — the T8 grid shows the trade
  (~+80 bp deep-tail carrot for ~+1.5 bp on mid-drain haircuts). Re-run T8/T9 first; every
...
| `Test_C1Battery_*` (six contracts in `Test_C1FullBattery.t.sol`) | T9 full battery re-run on the selected C1 band: T2 arb/cadence grids, flow breakeven sweep, T3 statics + year sim, T5/T6 whale + genesis (incl. min-fee shield diagnostic), T7 monthly-ST marks |
