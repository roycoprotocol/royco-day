...
contract EclpTestRouter {
    ...
    function addLiquidityUnbalanced(
        ...
        external
        returns (uint256[] memory amountsIn, uint256 bptOut)
    {
        return abi.decode(VAULT.unlock(abi.encodeCall(this.addLiquidityUnbalancedHook, (pool, payer, tokens, exactAmountsIn, minBpt))), (uint256[], uint256));
    }
        ...
        returns (uint256[] memory amountsIn, uint256 bptOut)
    {
        (amountsIn, bptOut,) = VAULT.addLiquidity(
            AddLiquidityParams({
                pool: pool, to: payer, maxAmountsIn: exactAmountsIn, minBptAmountOut: minBpt, kind: AddLiquidityKind.UNBALANCED, userData: ""
            })
        );
        for (uint256 i = 0; i < tokens.length; ++i) {
            if (amountsIn[i] > 0) {
    ...
    function removeLiquiditySingleTokenExactIn(
        ...
        returns (uint256 amountOut)
    {
        return abi.decode(VAULT.unlock(abi.encodeCall(this.removeSingleHook, (pool, payer, bptIn, tokens, outIndex, minOut))), (uint256));
    }
        ...
        returns (uint256 amountOut)
    {
        ...
        (, uint256[] memory amountsOut,) = VAULT.removeLiquidity(
            RemoveLiquidityParams({
                pool: pool, from: payer, maxBptAmountIn: bptIn, minAmountsOut: mins, kind: RemoveLiquidityKind.SINGLE_TOKEN_EXACT_IN, userData: ""
            })
        );
        for (uint256 i = 0; i < tokens.length; ++i) {
    ...
    function removeLiquidityProportional(address pool, address payer, uint256 bptIn, IERC20[] memory tokens) external returns (uint256[] memory amountsOut) {
        return abi.decode(VAULT.unlock(abi.encodeCall(this.removeProportionalHook, (pool, payer, bptIn, tokens))), (uint256[]));
    }

    function removeProportionalHook(
        address pool,
        address payer,
        uint256 bptIn,
        IERC20[] memory tokens
        ...
        returns (uint256[] memory amountsOut)
    {
        ...
        (, amountsOut,) = VAULT.removeLiquidity(
            RemoveLiquidityParams({ pool: pool, from: payer, maxBptAmountIn: bptIn, minAmountsOut: mins, kind: RemoveLiquidityKind.PROPORTIONAL, userData: "" })
        );
        for (uint256 i = 0; i < tokens.length; ++i) {
            if (amountsOut[i] > 0) VAULT.sendTo(tokens[i], payer, amountsOut[i]);
        }
...
abstract contract ECLPExitLiquidityBase is Test {
    ...
    function _derivedParamsA() internal pure returns (IGyroECLPPool.DerivedEclpParams memory) {
        return IGyroECLPPool.DerivedEclpParams({
            tauAlpha: IGyroECLPPool.Vector2({ x: -94_861_212_813_096_057_289_512_505_574_275_160_548, y: 31_644_119_574_235_279_926_451_292_677_567_331_631 }),
            tauBeta: IGyroECLPPool.Vector2({ x: 9_485_361_032_798_927_341_480_843_340_483_755, y: 99_999_999_550_139_629_318_738_613_277_634_649_855 }),
    ...
    function _derivedParamsD() internal pure returns (IGyroECLPPool.DerivedEclpParams memory) {
        return IGyroECLPPool.DerivedEclpParams({
            tauAlpha: IGyroECLPPool.Vector2({ x: -94_861_212_813_096_057_289_512_505_574_275_160_548, y: 31_644_119_574_235_279_926_451_292_677_567_331_631 }),
            tauBeta: IGyroECLPPool.Vector2({ x: 10_538_375_191_828_384_953_081_685_675_093_575_910, y: 99_443_162_903_822_885_390_594_315_002_629_343_516 }),
    ...
    function _arbToFeeEdge() internal returns (uint256 total) {
        for (uint256 round = 0; round < ARB_MAX_ROUNDS; ++round) {
            ...
            int256 realized = (int256(st.balanceOf(arber)) - stBefore) * int256(_fairStRate()) / 1e18 + (int256(quoteToken.balanceOf(arber)) - qBefore)
                * int256(_fairQRate()) / 1e18;
...
contract Test_PoolEconomics_ECLPExitLiquidity is ECLPExitLiquidityBase {
    ...
    function _runSyncDailyAtAnchor(uint256 t, uint256 i) internal returns (uint256 event1, uint256 steadyBpYrE4) {
        ...
        _logMetric(
            ...
            string.concat(
                ...
                "|drain=",
                _anchorLabel(i),
                "|cadence=24h|fee_bp_e4=10000|jump_bp_e4=",
                _i(jumps[0]),
                "|equil_recycle=",
                _u(equil0),
                "|event1_recycle=",
    ...
    function _runSync12hAtD50(uint256 t) internal returns (uint256 event1) {
        ...
        for (uint256 e = 0; e < 7; ++e) {
            assertApproxEqAbs(jumps[e], int256(6849), 100, "the sync-12h fair/oracle jump must be 0.6849 bp");
            if (e >= 1) {
        ...
        _logMetric(
            ...
            string.concat(
                "pool=",
                _tiltLabel(t),
                "|drain=D50|cadence=12h|fee_bp_e4=10000|jump_bp_e4=",
                _i(jumps[0]),
                "|equil_recycle=",
                _u(equil0),
                "|event1_recycle=",
                _u(profits[0]),
                "|steady_events_profit=0|arb_bp_yr_e4=0"
            )
        );
    }
    ...
    function _runAsyncCadenceAtAnchor(uint256 t, uint256 anchor) internal returns (uint256 steadyBpYrE4) {
        ...
        for (uint256 d = 2; d < 7; ++d) {
            ...
            assertApproxEqAbs(qJumps[d], int256(2740), 100, "the async quote-update fair/oracle gap must be ~0.27 bp (12h ST + 24h quote staleness)");
        ...
        _logMetric(
            ...
            string.concat(
                ...
                "|drain=",
                _anchorLabel(anchor),
                "|cadence=async12h|fee_bp_e4=10000|st_jump_bp_e4=",
                _i(stJumps[2]),
                "|q_jump_bp_e4=",
                _i(qJumps[2]),
                "|equil_recycle=",
    ...
    function _runWeeklyCadenceAtD50(uint256 t) internal returns (uint256 bpYrE4) {
        ...
        _logMetric(
            ...
            string.concat(
                "pool=",
                _tiltLabel(t),
                "|drain=D50|cadence=st1d_q7d|fee_bp_e4=10000|weekly_event_jump_bp_e4=",
                _i(weeklyEventJump),
                "|q_staleness_bp_e4=",
                _u(qStaleE4),
                "|equil_recycle=",
    ...
    function _runStaticBreakevenGrid(uint256 t) internal returns (uint256[4] memory event1) {
        ...
        _logMetric(
            "T2_BREAKEVEN",
            string.concat("pool=", _tiltLabel(t), "|cadence=24h|fee_bp_e4=15000|event1_recycle=", _u(feeProfits[0]), "|steady_event=", _u(feeProfits[2]))
        );
    }
    ...
    function _flowBreakevenSweep(uint256 t) internal returns (uint256[9] memory bpYr) {
        ...
        if (t == 0) {
            _logVerdict(
                ...
                string.concat(
                    "days_e2_at_0.5bp=", _u(be05), "|days_e2_at_1bp=", _u(be10), "|days_e2_at_1.5bp=", _u(be15), "|sub_breakeven_leak=impact_crumbs_only"
                )
            );
        }
    }
    ...
    function _runStandingDiscountAtAnchor(uint256 t, uint256 i) internal returns (uint256 profit) {
        ...
        if (i == 0) {
            ...
            assertLe(_bpE4(profit, tvl), 5000, "the D25 standing discount (1.22 bp) barely exceeds the 1 bp fee: profit < 0.5 bp of TVL");
        } else {
    ...
    function test_T3_SingleSidedStableAdd_OneWayCost_AcrossDrainStatesAndSizes() public {
        ...
        for (uint256 i = 0; i < 5; ++i) {
            ...
            for (uint256 sIdx = 0; sIdx < 3; ++sIdx) {
                _logPaired(
                    "T3_ADD_cost_spot_bp_e4", string.concat("drain=", _anchorLabel(i), "|size_pct_e4=", _u(sizePctWad[sIdx] / 1e12)), costA[sIdx], costD[sIdx]
                );
            }
        }
    }
    ...
    function _runAddCostsAtAnchor(uint256 t, uint256 i, uint256[3] memory sizePctWad) internal returns (int256[3] memory costSpots) {
        ...
        for (uint256 sIdx = 0; sIdx < 3; ++sIdx) {
            ...
            if (i == 0 && t == 0) {
                assertLt(costFair, 1000, "a single-sided quote add at the 99.99%-stable balance point must cost < 0.1 bp at fair");
    ...
    function test_T3_SingleSidedStableRoundTrip_CostAcrossDrainStates() public {
        ...
        _logVerdict(
            "Q3_round_trip", "meaningful_loss=false_both_tilts", string.concat("balance_point_roundTrip_bp_e4_tilt9999=", _i(d0A), "|tilt9010=", _i(d0D))
        );
    }
    ...
    function _runRoundTripAtAnchor(uint256 t, uint256 i) internal returns (int256 roundTripE4) {
        ...
        _logMetric(
            "T3_ROUNDTRIP",
            string.concat("pool=", _tiltLabel(t), "|drain=", _anchorLabel(i), "|oneWay_spot_bp_e4=", _i(costSpot), "|roundTrip_bp_e4=", _i(roundTripE4))
        );
    }
    ...
    function _runStandardWiring(uint256 t) internal returns (uint256 deadDay) {
        ...
        for (uint256 d = 1; d <= 30; ++d) {
            ...
            _logMetric("T4_WIRING", string.concat("pool=", _tiltLabel(t), "|day=", _u(d), "|spot=", _u(_spotPrice()), "|stable_share_e18=", _u(share)));
        }
    ...
    function _runOneWayEdge(uint256 t) internal returns (uint256 buyableBpTvlE4, uint256 haircutE4) {
        ...
        _logMetric(
            ...
            string.concat(
                ...
                "|max_buyable_st=",
                _u(xRaw),
                "|max_buyable_bp_tvl_e4=",
                _u(buyableBpTvlE4),
                "|one_shot_exit_1M_haircut_bp_e4=",
                _u(haircutE4)
            )
        );
    }
    ...
    function _runMinTradeAndRounding(uint256 t) internal returns (uint256 leakE4) {
        ...
        _restoreState(snap, snapTs);
        _logMetric("T4_EDGE", string.concat("pool=", _tiltLabel(t), "|dust_sell_2e6_out=", _u(out), "|d50_roundtrip_10k_leak_bp_e4=", _u(leakE4)));
    }
    ...
    function _runNumericsYear(uint256 t) internal returns (uint256 finalSpot, int256 costSpot) {
        ...
        _logMetric(
            ...
            string.concat(
                ...
                "|final_q_rate=",
                _u(qMarkEnd),
                "|final_stable_share_e18=",
                _u(stableShare),
                "|final_add_cost_spot_bp_e4=",
                _i(costSpot)
            )
        );
    }
...
contract Test_YearSimulation_ECLPExitLiquidity is ECLPExitLiquidityBase {
    ...
    function _runYearSim(uint256 t) internal returns (SimRes memory res) {
        ...
        int256 explained =
            (res.transferSum - int256(res.arbSum)) * int256(lpShareWad) / 1e18 + int256(res.carrySum) - int256(res.addCost) - int256(res.removeCost);
        res.residual = lpExcess - explained;
        res.avgTvl = sumTvl / 365;
        res.arbBpYrE4 = _bpE4(res.arbSum, res.avgTvl);

        assertLe(res.residual >= 0 ? res.residual : -res.residual, int256(deposit / 1e4), "the fair-value ledger must close to within 1 bp of the deposit");
...
contract Test_WhaleAddAndGenesis_ECLPExitLiquidity is ECLPExitLiquidityBase {
    ...
    function _freshPool(uint256 t, uint256 saltNum, uint256 x0, uint256 y0) internal {
        address p = _createPool(t == 0 ? _eclpParamsA() : _eclpParamsD(), t == 0 ? _derivedParamsA() : _derivedParamsD(), false, bytes32(saltNum));
        vm.prank(lp);
    ...
    function _runWhaleAdd(uint256 t) internal returns (WhaleRes memory res) {
        ...
        assertLe(res.roundTripE4, 2 * res.slipE4 + 2000, "the whale round trip must not exceed twice the one-way slippage plus 0.2 bp of remove-fee slack");
    ...
    function _runLadder(uint256 t) internal returns (int256[5] memory slipE4, bool[5] memory ok) {
        ...
        for (uint256 k = 0; k < 5; ++k) {
            ...
            _logMetric(
                "T5_LADDER", string.concat("pool=", _tiltLabel(t), "|add_wei=", _u(sizes[k]), "|ok=", ok[k] ? "1" : "0", "|slippage_bp_e4=", _i(slipE4[k]))
            );
        }
        _restoreState(snap, snapTs);
    }
    ...
    function test_T6_OneSidedGenesis_StablesOnlyInit_FeeShieldsTheBand_HeadToHead() public {
        ...
        for (uint256 s = 0; s < 3; ++s) {
            ...
            _logPaired("T6_GENESIS_vs_balanced_bp_e4", string.concat("seed=", seedLabels[s], "|fee_bp_e4=10000"), a.lossBpE4 - cfA, d.lossBpE4 - cfD);
            _logPaired("T6_GENESIS_minfee_vs_balanced_bp_e4", string.concat("seed=", seedLabels[s], "|fee_bp_e4=100"), am.lossBpE4 - cfAm, dm.lossBpE4 - cfDm);
        }
        ...
        _logVerdict(
            ...
            string.concat(
                "beta_minus_1_bp_e4_tilt9999=5|tilt9010=5299|fee_bp_e4=10000|minfee_diag_loss_bp_e4_tilt9010=", _i(minFeeLossD[1]), "|scale_invariant=true"
            )
        );
    }
...
contract Test_ExtremeCadence_ECLPExitLiquidity is ECLPExitLiquidityBase {
    ...
    function test_T7_ExtremeCadence_PerSecondQuote_MonthlyStMarks_HeadToHead() public {
        ...
        for (uint256 t = 0; t < 2; ++t) {
            ...
            assertGe(r.pinnedCheckpoints, (CYCLES * CHECKPOINTS_PER_CYCLE * 8) / 10, "the pool must sit pinned and inert for >= 80% of the horizon");
            /**
             ...
             */
            uint256 expectedCarry = r.strippedRaw.mulDown(uint256(5e16)) * (CYCLES * CYCLE - r.timeToStrip) / 365 days;
        ...
        _logVerdict(
            ...
            string.concat(
                "haircut_day15_bp_e4_9999=",
                _i(a.exiterHaircutE4),
                "|haircut_day15_bp_e4_9010=",
                _i(d.exiterHaircutE4),
                "|lp_net_bp_e4_9999=",
                _i(a.lpNetWei * 1e8 / int256(a.tvl0)),
                "|lp_net_bp_e4_9010=",
                _i(d.lpNetWei * 1e8 / int256(d.tvl0)),
