...
import { cellI } from "../../base/fixtures/TokenConfigs.sol";
import { TrancheFixtureSmoke } from "./TrancheFixtureSmoke.sol";

/**
 * @title CellISmokeNightlyCells
 * @notice Smoke battery on cell I: an 8-decimal quote stable against the baseline 4626(18,18) ST/JT shares
 * @dev The inherited battery's exact constants hold unchanged because every constant is denominated in WAD NAV,
 *      tranche shares, or BPT (all 18-decimal in every cell). The quote's 8 decimals surface only in the pool's
 *      raw token balances, pinned by the arithmetic below
 * @dev Nightly-only concrete, matched by the shared NightlyCells contract-name suffix
 *      (forge test --match-contract NightlyCells)
 */
contract CellISmokeNightlyCells is TrancheFixtureSmoke {
    function _smokeCell() internal pure override returns (FixtureCell memory) {
        return cellI();
    }

    /**
     * @dev Hand derivation for cell I: one whole ST asset = 1e18 share-wei, at initialRateWAD 1.0 that converts to
     *      1e18 underlying-wei = 1.0 whole 18-decimal underlying, and the 1.0 oracle price maps one whole
     *      underlying to exactly 1e18 NAV wei. The quote decimals do not enter the ST quoter at all
     */
     ...
     *      matters: this is the axis the cell exists for — the fixture's seed helpers convert WAD NAV to quote
    ...
    function test_EightDecimalQuote_poolLegLandsInQuoteWeiWhileMarksStayWAD() public {
        ...
        // The kernel-owned depth those wei back is the same cell-independent WAD mark every cell seeds:
