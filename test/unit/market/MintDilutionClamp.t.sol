 ...
 *         (ii) a deposit into a wiped-to-zero junior tranche (the I17 dilution state) mints exactly the cap
 ...
 * @dev The wipe uses a -40% shared-rate move: jtRaw 30_000e18 -> 18_000e18 absorbs its own 12_000e18 loss, and
 *      coverage for the senior's 40_000e18 loss consumes exactly the remaining 18_000e18, so jtEffectiveNAV
 *      lands at exactly zero with the junior supply still outstanding; the (jtEff == 0 && stEff > 0) arm then
 *      forces PERPETUAL, so deposits stay enabled in the wiped state
 */
contract MintDilutionClampTest is TrancheFixture {
     ...
     */
    function test_Clamp_liveMarketDeposit_inertAndFairPriced() public {
        ...
        assertEq(minted, RoycoTestMath.sharesFor(value, jtEffBefore, supplyBefore), "mirror agreement at the fair-priced mint");
    }
     ...
     */
    function test_Clamp_wipedTrancheDeposit_mintsExactlyTheCapWithPreviewParity() public {
        ...
        assertEq(minted, RoycoTestMath.sharesFor(value, 0, supplyBefore), "mirror agreement at the clamped mint");
