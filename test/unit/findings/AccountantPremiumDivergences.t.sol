...
import { AccountantUnitHarness } from "../accountant/AccountantUnitHarness.sol";

/**
 * @title AccountantPremiumDivergencesTest
 * @notice Loud, first-class pins of the accountant-layer divergences between production and the CLAUDE.md
 *         product spec that only reproduce on the mock-kernel accountant harness (findings 13-15 of
 ...
 */
contract AccountantPremiumDivergencesTest is AccountantUnitHarness {
     ...
     *      capped at the harness max 0.1e18): a same-block gain of stGain = 5e11 is below the 1e12 dust, so
    ...
    function test_FINDING_15_fixedTermEndTimestamp_truncatesToUint32AndWrapsIntoPast() public {
        ...
        // A covered -10e18 senior loss enters FIXED_TERM (covUtil 0.6e18 < the 1.1e18 liquidation threshold)
