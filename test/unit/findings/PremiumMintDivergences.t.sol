...
/**
 * @title PremiumMintDivergencesTest
 * @notice Loud, first-class pins of the two liquidity-premium-mint behaviors that reproduce on the full mock
 *         market: the whitelist-transfer brick (a confirmed defect) and the griefed-reinvestment staged buffer
 *         (documented, intended behavior). Findings 11-12 of docs/testing/agent-notes/13-spec-divergence-findings.md
 ...
 */
contract PremiumMintDivergencesTest is TrancheFixture {
     ...
     *         premium mint still succeeds and the freshly minted senior shares stay staged in the kernel
     *         (ltOwnedSeniorTrancheShares), NOT deployed into ltRawNAV and NOT forfeited. This matches CLAUDE.md's
     *         staged-buffer design: "Un-deployed premium is held as a kernel-staged buffer of ST shares ... The
     *         premium is claimable, not forfeited" and "Tolerates reversions to ensure a tranche operation doesn't
     *         revert on a failing reinvestment"
     * @dev This pins the CURRENT (correct) behavior so a future change that either reverts the sync on a failed
     *      reinvestment, or silently drops the staged premium, fails loudly. An attacker forcing venue slippage
     *      only DEFERS deployment; the metric keeps reading under-provisioned (ltRawNAV excludes the staged pile)
     *      so the LDM keeps paying
     */
