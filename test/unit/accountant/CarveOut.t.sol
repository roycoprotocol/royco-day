...
/**
 * @title CarveOutTest
 * @notice Golden vectors for the sync-time carve-out: the premium/fee share mints priced over the retained
...
contract CarveOutTest is AccountantUnitHarness {
     ...
     */
    function test_CarveOut_nominalJointPricing() public pure {
        ...
        // RoycoTestMath cross-assert (the carveOut mirror)
    ...
    /**
     * Zero pre-sync supply mints both carve-outs 1:1 with their NAV values (ValuationLogic.sol:104), the
     ...
     * (FeeAndLiquidityPremiumLogic.sol:49-58), leaves the senior supply and the staged premium pile
     * untouched, and never attempts a reinvestment — the quiet path must be a true no-op
     */
    function test_CarveOut_zeroPremiumAndFees_noMintCalls() public {
     ...
     * the reinvestment attempt, the staged pile lands at pre + premShares - drained
     * = 5e18 + 2_407_897_905_128_822_537 - 1e18, so a partial deploy never strands or double-counts shares
     */
    function test_CarveOut_partialReinvestmentDrainsStagedPile() public {
        flp.ST_LEDGER().setTotalSupply(1000e18);
        flp.setLTOwnedSeniorTrancheShares(5e18);
    ...
    /**
     * The LT effective NAV (ValuationLogic.sol:73-91) is the raw pool depth plus the staged premium shares
