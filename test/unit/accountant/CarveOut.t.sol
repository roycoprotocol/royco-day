...
contract CarveOutTest is AccountantUnitHarness {
    ...
    /**
     * V2.1 (F11, FLP:88-104): nominal carve-out at the W7-shaped sync outputs, run at the default residual
     * eps = 1e6 — the clamp is provably inert here (bind iff 2.5e18 * 1e6 > 1038.25e18 * (1e18 - 1e6), i.e.
     * 2.5e24 > ~1.038e39, false for both legs), so the pre-clamp literals hold unchanged.
    ...
    /**
     * V2.2 (F9 edge amended by the mint-dilution clamp, VL:113+, FLP:98): degenerate premium + fee == stEff
     * (100% of the sync's senior effective NAV carved out) at the default residual eps = 1e6.
     * retained = 10e18 - 4e18 - 6e18 = 0 -> the 1-wei denominator branch, and both legs BIND the clamp:
     ...
     */
    function test_CarveOut_V22_degeneratePremiumPlusFeeEqualsSTEff_clampsBothLegs() public pure {
        (uint256 premShares, uint256 feeShares, uint256 supplyAfter) =
    ...
    function _assertI8(uint256 _stEff, uint256 _prem, uint256 _fee, uint256 _preSupply) internal pure {
        ...
        assertTrue(_prem * 1e6 <= denom * (WAD - 1e6), "I8 precondition: the premium leg must not bind");
        assertTrue(_fee * 1e6 <= denom * (WAD - 1e6), "I8 precondition: the fee leg must not bind");
    ...
    /**
     * V2.6 (I8): the two-sided mint-value bound at adversarial non-binding (stEff, prem, fee, supply) tuples.
     * Hand-derived worked exemplars (all provably below the bind: legNAV * 1e6 <= denom * (1e18 - 1e6)):
    ...
    /**
     * V2.8 (FLP:66-72, VL:113+, strategy §4.1-9): an LT protocol fee equal to the entire LT effective NAV (the
     * 100% fee ceiling) drives the fee-share denominator ltEff - fee to zero, which routes through the F9
     * 1-wei branch — and at the default residual 1e6 the mint BINDS the dilution clamp.
     ...
     */
    function test_CarveOut_V28_ltFeeMintAtFullFee_clampsThroughOneWeiDenominator() public {
        flp.ST_LEDGER().setTotalSupply(1000e18);
        ...
        // The clamped 1-wei branch matches the RTM F9 mirror at the same residual
