...
/**
 * @title TrancheTransferAuthFuzz
 * @notice Fuzzes the full share-transfer authorization matrix on all three tranches: every transfer routes
 *         through the kernel's pre-balance-update hook, which screens the caller, sender, and receiver
 *         against the market's blacklist and (when the market enforces it) requires the receiver to be a
 *         whitelisted depositor for the tranche
 ...
 */
contract TrancheTransferAuthFuzz is TrancheFixture {
     ...
     */
    function testFuzz_TrancheTransfer_revertsExactlyWhenPartyBlacklistedOrReceiverNotWhitelisted(
        uint256 _trancheIdx,
        uint256 _fromIdx,
        uint256 _toIdx,
        uint256 _callerIdx,
