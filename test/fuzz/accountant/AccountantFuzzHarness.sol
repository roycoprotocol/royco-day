...
import { AccountantUnitHarness } from "../../unit/accountant/AccountantUnitHarness.sol";

/**
 * @title AccountantFuzzHarness
 * @notice Shared mock-kernel fuzz harness for the accountant property suite, extending the unit harness's
 ...
 */
abstract contract AccountantFuzzHarness is AccountantUnitHarness {
     ...
     *      mirror-waterfall input for the sync about to run against (_stRawNew, _jtRawNew, _ltRawNew) marks.
    ...
    {
        IRoycoDayAccountant.RoycoDayAccountantState memory s = accountant.getState();
        in_.stRawLast = toUint256(s.lastSTRawNAV);
        in_.jtRawLast = toUint256(s.lastJTRawNAV);
        in_.stEffLast = toUint256(s.lastSTEffectiveNAV);
        in_.jtEffLast = toUint256(s.lastJTEffectiveNAV);
        in_.jtCoverageILLast = toUint256(s.lastJTCoverageImpermanentLoss);
        in_.marketStateLast = RoycoTestMath.MarketState(uint8(s.lastMarketState));
        in_.fixedTermEndLast = s.fixedTermEndTimestamp;
        in_.stRawDelta = int256(_stRawNew) - int256(in_.stRawLast);
        in_.jtRawDelta = int256(_jtRawNew) - int256(in_.jtRawLast);
        in_.ltRawNew = _ltRawNew;
