...
/**
 * @title MaxActionInversionsFuzz
 * @notice Fuzzes the three gated capacity reads through the full production stack (tranche -> kernel -> accountant):
 ...
 */
contract MaxActionInversionsFuzz is MarketFuzzBase {
     ...
     * boundary can enter.
     *
     * Derivation (flat marks stRaw = stEff = st, jtRaw = jtEff = jt, ltRaw = depth, exact because 0.2e18 and
     * 0.05e18 divide WAD):
    ...
    function testFuzz_MaxSeniorDeposit_fillsCapacityExactlyAndOneMoreWeiReverts(uint256 _stSeed, uint256 _jtSeed, uint256 _extraQuoteSeed) public {
        ...
        assertLe(RoycoTestMath.covUtil(st + boundary, jt, true, 0.2e18, jt), WAD, "coverage utilization must sit at or below 100% exactly at the boundary");
        assertLe(RoycoTestMath.liqUtil(st + boundary, 0.05e18, depth), WAD, "liquidity utilization must sit at or below 100% exactly at the boundary");
    ...
    function testFuzz_MaxJuniorRedemption_drainsCoverageSurplusAndOneMoreShareReverts(uint256 _stSeed, uint256 _jtSeed) public {
        ...
        assertLe(
            RoycoTestMath.covUtil(st, jt - reportedMax, true, 0.2e18, jt - reportedMax),
            WAD,
    ...
    function testFuzz_MaxLiquidityRedemption_drainsToLiquidityFloorAndOneMoreShareReverts(uint256 _stSeed, uint256 _jtSeed, uint256 _extraQuoteSeed) public {
        ...
        assertEq(
            RoycoTestMath.maxLTWithdrawal(depth, st, 0.05e18, 1, RoycoTestMath.covUtil(st, jt, true, 0.2e18, jt), 6.4667e18),
            expectedMax,
        ...
        assertLe(RoycoTestMath.liqUtil(st, 0.05e18, requiredFloor), WAD, "liquidity utilization must sit at or below 100% exactly at the floor");
