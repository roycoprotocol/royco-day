...
/**
 * @title MarketFuzzBase
 * @notice Shared base for the market-level fuzz suites: cell A tokens (18-decimal ST/JT vault shares over an
 *         18-decimal underlying, 6-decimal quote stable) at the default market parameterization
 * @dev setUp only deploys (it re-runs per fuzz run), all seeding happens inside each test with fuzzed sizes.
 *      In cell A at a 1.0 vault rate and a 1.0 oracle price one vault-share wei is worth exactly one NAV wei,
 *      so seed sizes written in NAV wei are also exact tranche-unit amounts and initial share mints are 1:1
 */
abstract contract MarketFuzzBase is TrancheFixture {
    using Math for uint256;

    /// @dev NAV wei per quote wei at a 1.0 quote price (18 NAV decimals over the 6-decimal cell A quote)
     ...
     *      mirroring TrancheFixture._seedLT's quote-only leg placement
     */
