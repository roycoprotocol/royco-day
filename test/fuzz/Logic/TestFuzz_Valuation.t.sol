...
contract TestFuzz_Valuation_Logic is Test {
    ...
    /// @notice Virtual shares / virtual assets, restated inline independently of production and the mirror
    ...
    ///      effective supply cancels, so the predicate carries only the virtual-assets denominator). Products fit:
    ...
    function testFuzz_ConvertToShares_MatchesMirrorIncludingZeroEdges(uint256 _value, uint256 _totalValue, uint256 _supply) public pure {
        ...
        // shares / virtual assets there are three branches: the fresh-tranche 1:1 bootstrap (no shares AND no
    ...
    function testFuzz_ValuationRoundTrip_LossWithinDerivedSlack(uint256 _value, uint256 _totalValue, uint256 _supply) public pure {
        ...
        // Derived bound under virtual shares / virtual assets (both conversions floor): value >= v - ceil((T+VA)/(S+VS)) - 1
