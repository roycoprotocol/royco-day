...
library TemplateScaffold {
    ...
    function standUp(RoycoAccessManager _am, RoycoFactory _factory) internal returns (Result memory r) {
        ...
        r.template = RoycoDayBalancerV3MarketDeploymentTemplate(deployTemplateFor(_am, _factory, r.impls));
        ...
        r.market = new DeployMarketComponent(
            MarketUpstream({
                accessManager: address(_am), factory: address(_factory), entryPoint: address(0), marketSyncer: address(0), template: address(r.template)
            })
        );
    }
    ...
    function deployTemplateFor(RoycoAccessManager _am, RoycoFactory _factory, ImplementationSet memory _impls) internal returns (address template) {
        DeployTemplateComponent templateComponent =
            new DeployTemplateComponent(false, TemplateUpstream({ accessManager: address(_am), factory: address(_factory), impls: _impls }));
