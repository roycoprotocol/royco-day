...
contract DayMarketHandler is DayMarketTestBase {
    ...
    // The OZ virtual-shares share price floor((effNAV + VIRTUAL_ASSETS) * WAD / (supply + VIRTUAL_SHARES)) captured
    ...
    /// @dev The OZ virtual-shares share price floor((effNAV + VIRTUAL_ASSETS) * WAD / (supply + VIRTUAL_SHARES)),
    ...
    function _virtualSharePrice(uint256 _effNAV, uint256 _supply) internal pure returns (uint256) {
        return (_effNAV + RoycoTestMath.VIRTUAL_ASSETS).mulDiv(WAD, _supply + RoycoTestMath.VIRTUAL_SHARES);
    }
    ...
    ///      carries the virtual-shares/assets offset: floor((stEff + VIRTUAL_ASSETS) * WAD / (stSupply + VIRTUAL_SHARES)).
    ...
    function _mirrorSeniorRate(uint256 _stEff, uint256 _stSupply) internal pure returns (uint256) {
        ...
        uint256 r = (_stEff + RoycoTestMath.VIRTUAL_ASSETS).mulDiv(WAD, _stSupply + RoycoTestMath.VIRTUAL_SHARES);
        return r == 0 ? 1 : r;
    }
