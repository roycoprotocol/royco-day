...
/**
 * @title Test_Cache
 * @notice The unified keyed transient cache the venues share: a populated-marker bit (bit 255) distinguishes a
 ...
 *      driven through ONE external self-call, modeling production where a venue writes and reads the cache within
...
contract Test_Cache is Test {
     ...
     *      asset rate, or a venue would price off a value meant for a different quantity
     */
     ...
     *      untouched. This is the read-after-write identity at its upper edge: a venue that caches a rate must
