...
contract Test_FixedYDM is Test {
    ...
    function _referenceShare() internal returns (FixedYDM ydm) {
        ydm = new FixedYDM();
        ydm.initializeYDMForMarket(3e17);
    }
    ...
    function test_RevertIf_InitializeShareAboveWad() public {
        ...
        ydm.initializeYDMForMarket(uint64(WAD + 1));
    }
    ...
    function test_RevertIf_InitializeShareUint64Max() public {
        FixedYDM ydm = new FixedYDM();
        vm.expectRevert(IYDM.INVALID_YDM_INITIALIZATION.selector);
        ydm.initializeYDMForMarket(type(uint64).max);
    }
    ...
    function test_Initialize_ShareAtWadAllowed() public {
        FixedYDM ydm = new FixedYDM();
        ydm.initializeYDMForMarket(uint64(WAD));
        assertEq(ydm.previewYieldShare(MarketState.PERPETUAL, 0), WAD, "Y == WAD everywhere");
    }
    ...
    function test_Initialize_ZeroShareAllowed() public {
        FixedYDM ydm = new FixedYDM();
        ydm.initializeYDMForMarket(0);

        (bool initialized, uint64 shareWAD) = ydm.accountantToFixedYieldShare(address(this));
        ...
        assertEq(ydm.previewYieldShare(MarketState.PERPETUAL, WAD), 0, "Y(WAD) == 0");
        assertEq(ydm.previewYieldShare(MarketState.FIXED_TERM, type(uint256).max), 0, "Y is zero at any utilization and state");
    }

    /// Valid init emits FixedYdmInitialized(accountant, share) and stores the packed state
    function test_Initialize_ReferenceShare_EmitsAndStores() public {
        FixedYDM ydm = new FixedYDM();
        vm.expectEmit(true, true, true, true, address(ydm));
        emit FixedYdmInitialized(address(this), 3e17);
        ydm.initializeYDMForMarket(3e17);
    ...
    function test_Initialize_ReinitOverwrites() public {
        ...
        ydm.initializeYDMForMarket(7e17);
        assertEq(ydm.previewYieldShare(MarketState.PERPETUAL, 5e17), 7e17, "the second init's share wins");
    }
    ...
    function test_YieldShare_IgnoresUtilization() public {
        FixedYDM ydm = _referenceShare();
        assertEq(ydm.previewYieldShare(MarketState.PERPETUAL, 0), 3e17, "Y(0)");
        assertEq(ydm.previewYieldShare(MarketState.PERPETUAL, 5e17), 3e17, "Y(0.5)");
        assertEq(ydm.previewYieldShare(MarketState.PERPETUAL, WAD), 3e17, "Y(WAD)");
        assertEq(ydm.previewYieldShare(MarketState.PERPETUAL, type(uint256).max), 3e17, "Y(uint max)");
    }
    ...
    function test_YieldShare_IgnoresMarketState() public {
        FixedYDM ydm = _referenceShare();
        assertEq(ydm.previewYieldShare(MarketState.PERPETUAL, 5e17), 3e17, "PERPETUAL");
        assertEq(ydm.previewYieldShare(MarketState.FIXED_TERM, 5e17), 3e17, "FIXED_TERM");
    }
    ...
    function test_YieldShare_MatchesPreviewAndEmits() public {
        FixedYDM ydm = _referenceShare();
        uint256 previewed = ydm.previewYieldShare(MarketState.PERPETUAL, 5e17);
        vm.expectEmit(true, true, true, true, address(ydm));
        emit YdmOutput(address(this), 3e17);
        assertEq(ydm.yieldShare(MarketState.PERPETUAL, 5e17), previewed, "yieldShare == previewYieldShare");
    }
    ...
    function test_YieldShare_InitializedSenderCanQuery() public {
        FixedYDM ydm = _referenceShare();
        assertEq(ydm.yieldShare(MarketState.PERPETUAL, 0), 3e17, "reference share Y(0)");
    }
    ...
    function test_ZeroShareInitialized_DistinctFromUninitialized() public {
        FixedYDM ydm = new FixedYDM();
        ydm.initializeYDMForMarket(0);
        assertEq(ydm.yieldShare(MarketState.PERPETUAL, 5e17), 0, "the initialized zero share reads zero");
        vm.prank(ACCT_B);
        vm.expectRevert(IYDM.UNINITIALIZED_YDM.selector);
        ydm.yieldShare(MarketState.PERPETUAL, 5e17);
    }
