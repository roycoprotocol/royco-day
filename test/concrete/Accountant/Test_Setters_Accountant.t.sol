...
contract Test_Setters_Accountant is AccountantTestBase {
     ...
     * once:
     * 1. Protocol fees: a genuine 100e18 collateral gain still pays the JT risk premium and LT liquidity premium,
     ...
     *    the IL ledger at commit — the fixed-term protection window never engages AND the senior never owes
    ...
    function test_HugeDustTolerance_SuppressesProtocolFeesPremiumResetsAndFixedTermEntry() public {
        ...
        // twLT = 0.05e18 * 1000 = 50e18 pay jtRiskPremium = floor(deltaST * 100e18 / (1000 * 1e18)) = 8333333333333333333
        // and ltLiquidityPremium = 4166666666666666666 out of the senior gain, so
        // jtEffectiveNAV = 200e18 + 16666666666666666667 + 8333333333333333333 = 225e18 and
        // stEffectiveNAV = 1000e18 + (deltaST - jtPrem - ltPrem) + ltPrem = 1075e18 (conservation: 1300e18)
        ...
        assertEq(toUint256(state.ltLiquidityPremium), 4_166_666_666_666_666_666, "lt liquidity premium is still paid");
        ...
        assertEq(toUint256(state.ltProtocolFee), 0, "lt fee skipped because the gain reads as dust");
        ...
        s = accountant.getState();
        assertEq(uint8(s.lastMarketState), uint8(MarketState.PERPETUAL), "fixed-term protection never engages");
     ...
     * jtPrem = floor(deltaST * 200e18 / (1000 * 1e18)) = 16666666666666666666 and ltPrem = 8333333333333333333:
     *   jtEffectiveNAV = 200e18 + 16666666666666666667 + 16666666666666666666 = 233333333333333333333
     *   stEffectiveNAV = 1000e18 + (deltaST - jtPrem - ltPrem) + ltPrem = 1066666666666666666667
     *   jtFee = floor(16666666666666666667 * 0.1) + floor(jtPrem * 0.1) = 3333333333333333332
     *   ltFee = floor(ltPrem * 0.1) = 833333333333333333, stFee = floor(58333333333333333334 * 0.1) = 5833333333333333333
     */
    function test_SetMaxYieldShares_loweringCapDoesNotEraseAccruedWindow() public {
        ...
        assertEq(toUint256(state.ltLiquidityPremium), 8_333_333_333_333_333_333, "lt premium priced from the pre-change window");
        ...
        assertEq(toUint256(state.ltProtocolFee), 833_333_333_333_333_333, "lt fee on the earned premium");
