 ...
 *         downstream effects
 * @notice The LPT flows are split per variant: LPT_DEPOSIT and LPT_REDEEM are the in-kind ops that can only
 *         move the liquidity mark (and, for redeem, hand out idle premium shares), LPT_MULTI_ASSET_DEPOSIT
 *         and LPT_MULTI_ASSET_REDEEM are the underlying-asset ops that can also mint or unwind senior
...
contract Test_PostOpSync_Accountant is AccountantTestBase {
    ...
    function test_PostOp_STDeposit_addsDeltaToSTEffective() public {
        ...
        SyncedAccountingState memory state =
            kernel.doPostOp(Operation.ST_DEPOSIT, toNAVUnits(SEED_COLLATERAL + 123e18), toNAVUnits(SEED_LPT_RAW), ZERO_NAV_UNITS, false);
    ...
    function test_RevertIf_STDepositZeroCollateralDelta() public {
        ...
        kernel.doPostOp(Operation.ST_DEPOSIT, toNAVUnits(SEED_COLLATERAL), toNAVUnits(SEED_LPT_RAW), ZERO_NAV_UNITS, false);
    }
    ...
    function test_RevertIf_STDepositNegativeCollateralDelta() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.ST_DEPOSIT));
        kernel.doPostOp(Operation.ST_DEPOSIT, toNAVUnits(SEED_COLLATERAL - 1), toNAVUnits(SEED_LPT_RAW), ZERO_NAV_UNITS, false);
    }
    ...
    function test_RevertIf_STDepositNonzeroLPTDelta() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.ST_DEPOSIT));
        kernel.doPostOp(Operation.ST_DEPOSIT, toNAVUnits(SEED_COLLATERAL + 10e18), toNAVUnits(SEED_LPT_RAW + 1), ZERO_NAV_UNITS, false);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.ST_DEPOSIT));
        kernel.doPostOp(Operation.ST_DEPOSIT, toNAVUnits(SEED_COLLATERAL + 10e18), toNAVUnits(SEED_LPT_RAW - 1), ZERO_NAV_UNITS, false);
    }
    ...
    function test_RevertIf_STDepositNonzeroBonus() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.ST_DEPOSIT));
        kernel.doPostOp(Operation.ST_DEPOSIT, toNAVUnits(SEED_COLLATERAL + 10e18), toNAVUnits(SEED_LPT_RAW), toNAVUnits(uint256(1)), false);
    }
    ...
    function test_PostOp_JTDeposit_addsDeltaToJTEffective() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        SyncedAccountingState memory state =
            kernel.doPostOp(Operation.JT_DEPOSIT, toNAVUnits(SEED_COLLATERAL + 45e18), toNAVUnits(SEED_LPT_RAW), ZERO_NAV_UNITS, false);
    ...
    function test_RevertIf_JTDepositZeroCollateralDelta() public {
        ...
        kernel.doPostOp(Operation.JT_DEPOSIT, toNAVUnits(SEED_COLLATERAL), toNAVUnits(SEED_LPT_RAW), ZERO_NAV_UNITS, false);
    }
    ...
    function test_RevertIf_JTDepositNegativeCollateralDelta() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.JT_DEPOSIT));
        kernel.doPostOp(Operation.JT_DEPOSIT, toNAVUnits(SEED_COLLATERAL - 1), toNAVUnits(SEED_LPT_RAW), ZERO_NAV_UNITS, false);
    }
    ...
    function test_RevertIf_JTDepositNonzeroLPTDelta() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.JT_DEPOSIT));
        kernel.doPostOp(Operation.JT_DEPOSIT, toNAVUnits(SEED_COLLATERAL + 10e18), toNAVUnits(SEED_LPT_RAW + 1), ZERO_NAV_UNITS, false);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.JT_DEPOSIT));
        kernel.doPostOp(Operation.JT_DEPOSIT, toNAVUnits(SEED_COLLATERAL + 10e18), toNAVUnits(SEED_LPT_RAW - 1), ZERO_NAV_UNITS, false);
    }
    ...
    function test_RevertIf_JTDepositNonzeroBonus() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.JT_DEPOSIT));
        kernel.doPostOp(Operation.JT_DEPOSIT, toNAVUnits(SEED_COLLATERAL + 10e18), toNAVUnits(SEED_LPT_RAW), toNAVUnits(uint256(1)), false);
    }
    ...
    function test_PostOp_LPTDepositBPTOnly_leavesEffectiveNAVsUntouched() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        SyncedAccountingState memory state =
            kernel.doPostOp(Operation.LPT_DEPOSIT, toNAVUnits(SEED_COLLATERAL), toNAVUnits(SEED_LPT_RAW + 30e18), ZERO_NAV_UNITS, false);
    ...
    /// a multi-asset LPT deposit (positive collateral delta) adds the freshly minted senior value to the senior
    ...
    function test_PostOp_LPTMultiAssetDeposit_addsCollateralDeltaToSTEffective() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        SyncedAccountingState memory state =
            kernel.doPostOp(Operation.LPT_MULTI_ASSET_DEPOSIT, toNAVUnits(SEED_COLLATERAL + 50e18), toNAVUnits(SEED_LPT_RAW + 20e18), ZERO_NAV_UNITS, false);
        assertEq(toUint256(state.stEffectiveNAV), SEED_ST_EFF + 50e18, "st effective NAV grows by the minted senior value");
        assertEq(toUint256(state.lptRawNAV), SEED_LPT_RAW + 20e18, "lt raw NAV reflects the joined BPT value");
        assertEq(toUint256(accountant.getState().lastSTEffectiveNAV), SEED_ST_EFF + 50e18, "st effective NAV committed");
    }

    /// a quote-only multi-asset LPT deposit (zero collateral delta) passes and leaves both effective NAVs untouched,
    ...
    function test_PostOp_LPTMultiAssetDeposit_zeroCollateralDeltaPasses() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        SyncedAccountingState memory state =
            kernel.doPostOp(Operation.LPT_MULTI_ASSET_DEPOSIT, toNAVUnits(SEED_COLLATERAL), toNAVUnits(SEED_LPT_RAW + 30e18), ZERO_NAV_UNITS, false);
    ...
    function test_RevertIf_LPTDepositNonzeroCollateralDelta() public {
        ...
        kernel.doPostOp(Operation.LPT_DEPOSIT, toNAVUnits(SEED_COLLATERAL + 50e18), toNAVUnits(SEED_LPT_RAW + 20e18), ZERO_NAV_UNITS, false);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.LPT_DEPOSIT));
        kernel.doPostOp(Operation.LPT_DEPOSIT, toNAVUnits(SEED_COLLATERAL - 1), toNAVUnits(SEED_LPT_RAW + 10e18), ZERO_NAV_UNITS, false);
    }
    ...
    function test_RevertIf_LPTDepositZeroLPTDelta() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.LPT_DEPOSIT));
        kernel.doPostOp(Operation.LPT_DEPOSIT, toNAVUnits(SEED_COLLATERAL), toNAVUnits(SEED_LPT_RAW), ZERO_NAV_UNITS, false);
    }
    ...
    function test_RevertIf_LPTDepositNegativeLPTDelta() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.LPT_DEPOSIT));
        kernel.doPostOp(Operation.LPT_DEPOSIT, toNAVUnits(SEED_COLLATERAL), toNAVUnits(SEED_LPT_RAW - 1), ZERO_NAV_UNITS, false);
    }
    ...
    function test_RevertIf_LPTDepositNonzeroBonus() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.LPT_DEPOSIT));
        kernel.doPostOp(Operation.LPT_DEPOSIT, toNAVUnits(SEED_COLLATERAL), toNAVUnits(SEED_LPT_RAW + 10e18), toNAVUnits(uint256(1)), false);
    }

    /// a multi-asset LPT deposit with a negative collateral delta violates the shape require, the flow can
    ...
    function test_RevertIf_LPTMultiAssetDepositNegativeCollateralDelta() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.LPT_MULTI_ASSET_DEPOSIT));
        kernel.doPostOp(Operation.LPT_MULTI_ASSET_DEPOSIT, toNAVUnits(SEED_COLLATERAL - 1), toNAVUnits(SEED_LPT_RAW + 10e18), ZERO_NAV_UNITS, false);
    }

    /// a multi-asset LPT deposit with a zero or negative liquidity raw NAV delta violates the shape require, a
    /// liquidity deposit that added no pooled depth would mint LPT claims against nothing
    function test_RevertIf_LPTMultiAssetDepositNonpositiveLPTDelta() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.LPT_MULTI_ASSET_DEPOSIT));
        kernel.doPostOp(Operation.LPT_MULTI_ASSET_DEPOSIT, toNAVUnits(SEED_COLLATERAL + 10e18), toNAVUnits(SEED_LPT_RAW), ZERO_NAV_UNITS, false);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.LPT_MULTI_ASSET_DEPOSIT));
        kernel.doPostOp(Operation.LPT_MULTI_ASSET_DEPOSIT, toNAVUnits(SEED_COLLATERAL + 10e18), toNAVUnits(SEED_LPT_RAW - 1), ZERO_NAV_UNITS, false);
    }

    /// a multi-asset LPT deposit with a nonzero self-liquidation bonus value violates the shape require, the
    /// junior-funded bonus exists only on senior redemptions
    function test_RevertIf_LPTMultiAssetDepositNonzeroBonus() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.LPT_MULTI_ASSET_DEPOSIT));
        kernel.doPostOp(Operation.LPT_MULTI_ASSET_DEPOSIT, toNAVUnits(SEED_COLLATERAL), toNAVUnits(SEED_LPT_RAW + 10e18), toNAVUnits(uint256(1)), false);
    }
    ...
    function test_PostOp_STRedeem_reducesSTEffectiveWithoutBonus() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        SyncedAccountingState memory state =
            kernel.doPostOp(Operation.ST_REDEEM, toNAVUnits(SEED_COLLATERAL - 50e18), toNAVUnits(SEED_LPT_RAW), ZERO_NAV_UNITS, false);
    ...
    function test_PostOp_STRedeem_bonusSplitsAcrossJTAndST() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        SyncedAccountingState memory state =
            kernel.doPostOp(Operation.ST_REDEEM, toNAVUnits(SEED_COLLATERAL - 55e18), toNAVUnits(SEED_LPT_RAW), toNAVUnits(uint256(5e18)), false);
    ...
    function test_PostOp_STRedeem_bonusEqualToTotalDrawsAllFromJT() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        SyncedAccountingState memory state =
            kernel.doPostOp(Operation.ST_REDEEM, toNAVUnits(SEED_COLLATERAL - 10e18), toNAVUnits(SEED_LPT_RAW), toNAVUnits(uint256(10e18)), false);
    ...
    function test_RevertIf_STRedeemNonzeroLPTDelta() public {
        ...
        kernel.doPostOp(Operation.ST_REDEEM, toNAVUnits(SEED_COLLATERAL - 10e18), toNAVUnits(SEED_LPT_RAW + 1), ZERO_NAV_UNITS, false);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.ST_REDEEM));
        kernel.doPostOp(Operation.ST_REDEEM, toNAVUnits(SEED_COLLATERAL - 10e18), toNAVUnits(SEED_LPT_RAW - 1), ZERO_NAV_UNITS, false);
    }
    ...
    function test_RevertIf_STRedeemZeroTotal() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.ST_REDEEM));
        kernel.doPostOp(Operation.ST_REDEEM, toNAVUnits(SEED_COLLATERAL), toNAVUnits(SEED_LPT_RAW), ZERO_NAV_UNITS, false);
    }
    ...
    function test_RevertIf_STRedeemPositiveCollateralDelta() public {
        ...
        kernel.doPostOp(Operation.ST_REDEEM, toNAVUnits(SEED_COLLATERAL + 1), toNAVUnits(SEED_LPT_RAW), ZERO_NAV_UNITS, false);
    }
    ...
    function test_RevertIf_STRedeemBonusExceedsJTEffective() public {
        ...
        kernel.doPostOp(Operation.ST_REDEEM, toNAVUnits(SEED_ST_EFF + 5e18 - 10e18), toNAVUnits(SEED_LPT_RAW), toNAVUnits(uint256(6e18)), false);
    }
    ...
    function test_RevertIf_STRedeemBonusExceedsTotal() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(stdError.arithmeticError);
        kernel.doPostOp(Operation.ST_REDEEM, toNAVUnits(SEED_COLLATERAL - 10e18), toNAVUnits(SEED_LPT_RAW), toNAVUnits(uint256(11e18)), false);
    }
    ...
    function test_PostOp_LPTRedeem_negativeLPTDeltaAlonePasses() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        SyncedAccountingState memory state =
            kernel.doPostOp(Operation.LPT_REDEEM, toNAVUnits(SEED_COLLATERAL), toNAVUnits(SEED_LPT_RAW - 40e18), ZERO_NAV_UNITS, false);
    ...
    /**
     * a multi-asset LPT redemption with a zero liquidity delta but a positive total passes: the zero-BPT-slice
    ...
    function test_PostOp_LPTMultiAssetRedeem_zeroLPTDeltaWithPositiveTotalPasses() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        SyncedAccountingState memory state =
            kernel.doPostOp(Operation.LPT_MULTI_ASSET_REDEEM, toNAVUnits(SEED_COLLATERAL - 10e18), toNAVUnits(SEED_LPT_RAW), ZERO_NAV_UNITS, false);
    ...
    /// a multi-asset LPT redemption with both a negative liquidity delta and a positive total passes
    function test_PostOp_LPTMultiAssetRedeem_bothLegsPass() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        SyncedAccountingState memory state =
            kernel.doPostOp(Operation.LPT_MULTI_ASSET_REDEEM, toNAVUnits(SEED_COLLATERAL - 10e18), toNAVUnits(SEED_LPT_RAW - 40e18), ZERO_NAV_UNITS, false);
        assertEq(toUint256(state.stEffectiveNAV), SEED_ST_EFF - 10e18, "st effective NAV bears the unwound senior leg");
        assertEq(toUint256(state.lptRawNAV), SEED_LPT_RAW - 40e18, "lt raw NAV reflects the burned BPT slice");
    }

    /**
     * a multi-asset LPT redemption with a self-liquidation bonus reduces the junior effective NAV by exactly
    ...
    function test_PostOp_LPTMultiAssetRedeem_bonusSplitsAcrossJTAndST() public {
        ...
        SyncedAccountingState memory state = kernel.doPostOp(
            Operation.LPT_MULTI_ASSET_REDEEM, toNAVUnits(SEED_COLLATERAL - 55e18), toNAVUnits(SEED_LPT_RAW - 40e18), toNAVUnits(uint256(5e18)), false
        );
        assertEq(toUint256(state.jtEffectiveNAV), SEED_JT_EFF - 5e18, "jt effective NAV funds exactly the bonus");
        assertEq(toUint256(state.stEffectiveNAV), SEED_ST_EFF - 50e18, "st effective NAV bears the redemption net of the bonus");
        IRoycoDayAccountant.RoycoDayAccountantState memory s = accountant.getState();
    ...
    /// a multi-asset LPT redemption with a positive liquidity delta violates the shape require
    function test_RevertIf_LPTMultiAssetRedeemPositiveLPTDelta() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.LPT_MULTI_ASSET_REDEEM));
        kernel.doPostOp(Operation.LPT_MULTI_ASSET_REDEEM, toNAVUnits(SEED_COLLATERAL - 10e18), toNAVUnits(SEED_LPT_RAW + 1), ZERO_NAV_UNITS, false);
    }

    /**
     * a positive collateral delta during a multi-asset LPT redemption reverts inside toNAVUnits(int256) with
    ...
    function test_RevertIf_LPTMultiAssetRedeemPositiveCollateralDelta() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(ASSETS_MUST_BE_NON_NEGATIVE.selector);
        kernel.doPostOp(Operation.LPT_MULTI_ASSET_REDEEM, toNAVUnits(SEED_COLLATERAL + 1), toNAVUnits(SEED_LPT_RAW - 10e18), ZERO_NAV_UNITS, false);
    }
    ...
    function test_RevertIf_LPTMultiAssetRedeemBonusExceedsJTEffective() public {
        ...
        vm.expectRevert(stdError.arithmeticError);
        kernel.doPostOp(
            Operation.LPT_MULTI_ASSET_REDEEM, toNAVUnits(SEED_ST_EFF + 5e18 - 10e18), toNAVUnits(SEED_LPT_RAW - 1e18), toNAVUnits(uint256(6e18)), false
        );
    }
    ...
    function test_RevertIf_LPTRedeemNonzeroCollateralDelta() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.LPT_REDEEM));
        kernel.doPostOp(Operation.LPT_REDEEM, toNAVUnits(SEED_COLLATERAL - 10e18), toNAVUnits(SEED_LPT_RAW - 40e18), ZERO_NAV_UNITS, false);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.LPT_REDEEM));
        kernel.doPostOp(Operation.LPT_REDEEM, toNAVUnits(SEED_COLLATERAL + 1), toNAVUnits(SEED_LPT_RAW - 40e18), ZERO_NAV_UNITS, false);
    }
    ...
    function test_RevertIf_LPTRedeemNonzeroBonus() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.LPT_REDEEM));
        kernel.doPostOp(Operation.LPT_REDEEM, toNAVUnits(SEED_COLLATERAL), toNAVUnits(SEED_LPT_RAW - 40e18), toNAVUnits(uint256(1)), false);
    }
     ...
     */
    function test_PostOp_LPTRedeem_zeroLPTDeltaZeroTotalPasses() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        SyncedAccountingState memory state = kernel.doPostOp(Operation.LPT_REDEEM, toNAVUnits(SEED_COLLATERAL), toNAVUnits(SEED_LPT_RAW), ZERO_NAV_UNITS, false);
        assertEq(toUint256(state.lptRawNAV), SEED_LPT_RAW, "lt raw NAV untouched (no BPT slice burned)");
        assertEq(toUint256(state.stEffectiveNAV), SEED_ST_EFF, "st effective NAV untouched (senior shares stay in supply)");
        assertEq(toUint256(state.jtEffectiveNAV), SEED_JT_EFF, "jt effective NAV untouched");
        assertEq(toUint256(accountant.getState().lastLPTRawNAV), SEED_LPT_RAW, "committed lt raw NAV untouched");
    }
    ...
    function test_RevertIf_LPTRedeemPositiveLPTDelta() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.LPT_REDEEM));
        kernel.doPostOp(Operation.LPT_REDEEM, toNAVUnits(SEED_COLLATERAL), toNAVUnits(SEED_LPT_RAW + 1), ZERO_NAV_UNITS, false);
    }
    ...
    function test_PostOp_JTRedeem_reducesJTEffectiveWithZeroILUntouched() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        SyncedAccountingState memory state =
            kernel.doPostOp(Operation.JT_REDEEM, toNAVUnits(SEED_COLLATERAL - 50e18), toNAVUnits(SEED_LPT_RAW), ZERO_NAV_UNITS, false);
    ...
    function test_PostOp_JTRedeem_leavesILLedgerUntouched() public {
        ...
        SyncedAccountingState memory state = kernel.doPostOp(Operation.JT_REDEEM, toNAVUnits(uint256(1140e18)), toNAVUnits(SEED_LPT_RAW), ZERO_NAV_UNITS, false);
        ...
        state = kernel.doPostOp(Operation.JT_REDEEM, toNAVUnits(uint256(1140e18 - 7)), toNAVUnits(SEED_LPT_RAW), ZERO_NAV_UNITS, false);
    ...
    function test_RevertIf_JTRedeemNonzeroLPTDelta() public {
        ...
        kernel.doPostOp(Operation.JT_REDEEM, toNAVUnits(SEED_COLLATERAL - 10e18), toNAVUnits(SEED_LPT_RAW + 1), ZERO_NAV_UNITS, false);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.JT_REDEEM));
        kernel.doPostOp(Operation.JT_REDEEM, toNAVUnits(SEED_COLLATERAL - 10e18), toNAVUnits(SEED_LPT_RAW - 1), ZERO_NAV_UNITS, false);
    }
    ...
    function test_RevertIf_JTRedeemZeroTotal() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.JT_REDEEM));
        kernel.doPostOp(Operation.JT_REDEEM, toNAVUnits(SEED_COLLATERAL), toNAVUnits(SEED_LPT_RAW), ZERO_NAV_UNITS, false);
    }
    ...
    function test_RevertIf_JTRedeemPositiveCollateralDelta() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(ASSETS_MUST_BE_NON_NEGATIVE.selector);
        kernel.doPostOp(Operation.JT_REDEEM, toNAVUnits(SEED_COLLATERAL + 1), toNAVUnits(SEED_LPT_RAW), ZERO_NAV_UNITS, false);
    }
    ...
    function test_RevertIf_JTRedeemNonzeroBonus() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        vm.expectRevert(abi.encodeWithSelector(IRoycoDayAccountant.INVALID_POST_OP_STATE.selector, Operation.JT_REDEEM));
        kernel.doPostOp(Operation.JT_REDEEM, toNAVUnits(SEED_COLLATERAL - 10e18), toNAVUnits(SEED_LPT_RAW), toNAVUnits(uint256(1)), false);
    }
    ...
    function testFuzz_PostOp_conservationHoldsForValidShapes(uint256 _stEff0, uint256 _jtEff0, uint256 _lpt0, uint256 _value, uint256 _opSeed) public {
        ...
        kernel.doPostOp(op, toNAVUnits(collateral1), toNAVUnits(lpt1), bonus, false);
    ...
    function test_PostOp_writesAllCheckpointsAndPreservesMarketState() public {
        ...
        vm.recordLogs();
        SyncedAccountingState memory state =
            kernel.doPostOp(Operation.LPT_DEPOSIT, toNAVUnits(uint256(1200e18)), toNAVUnits(uint256(130e18)), ZERO_NAV_UNITS, false);
    ...
    function test_PostOp_enforceFalseSkipsBothGatesForEveryOp() public {
        ...
        // LPT_DEPOSIT under a persisting liquidity breach still passes (lt 15e18)
        state = kernel.doPostOp(Operation.LPT_DEPOSIT, toNAVUnits(uint256(1100e18)), toNAVUnits(uint256(15e18)), ZERO_NAV_UNITS, false);
        assertGt(state.liquidityUtilizationWAD, WAD, "liquidity still breached after the lt deposit");
        // LPT_REDEEM deepens the liquidity breach and still passes (lt 5e18)
        state = kernel.doPostOp(Operation.LPT_REDEEM, toNAVUnits(uint256(1100e18)), toNAVUnits(uint256(5e18)), ZERO_NAV_UNITS, false);
        assertGt(state.coverageUtilizationWAD, WAD, "coverage still breached after the lt redemption");
        // LPT_MULTI_ASSET_DEPOSIT deepens the coverage breach with a senior leg and still passes (collateral 1150e18, lt 6e18)
        state = kernel.doPostOp(Operation.LPT_MULTI_ASSET_DEPOSIT, toNAVUnits(uint256(1150e18)), toNAVUnits(uint256(6e18)), ZERO_NAV_UNITS, false);
        assertGt(state.coverageUtilizationWAD, WAD, "coverage still breached after the multi-asset lt deposit");
        // LPT_MULTI_ASSET_REDEEM deepens the liquidity breach with an unwound senior leg and still passes
        state = kernel.doPostOp(Operation.LPT_MULTI_ASSET_REDEEM, toNAVUnits(uint256(1100e18)), toNAVUnits(uint256(5e18)), ZERO_NAV_UNITS, false);
    ...
    /**
     * the coverage gate for LPT_MULTI_ASSET_DEPOSIT (senior-minting) passes at exactly WAD and fires at WAD + 1
    ...
    function test_PostOp_coverageGate_lptMultiAssetDepositExactBoundary() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        SyncedAccountingState memory state =
            kernel.doPostOp(Operation.LPT_MULTI_ASSET_DEPOSIT, toNAVUnits(uint256(2000e18)), toNAVUnits(uint256(150e18)), ZERO_NAV_UNITS, true);
        assertEq(state.coverageUtilizationWAD, WAD, "coverage utilization lands exactly on WAD and passes");
        vm.expectRevert(IRoycoDayAccountant.COVERAGE_REQUIREMENT_VIOLATED.selector);
        kernel.doPostOp(Operation.LPT_MULTI_ASSET_DEPOSIT, toNAVUnits(uint256(2000e18 + 1)), toNAVUnits(uint256(150e18 + 1)), ZERO_NAV_UNITS, true);
    }

    /// an in-kind LPT deposit passes an enforced coverage breach because pre-minted BPT can never add senior
    ...
    function test_PostOp_gateExemptions_lptDepositPassesCoverageBreach() public {
        ...
        SyncedAccountingState memory state =
            kernel.doPostOp(Operation.LPT_DEPOSIT, toNAVUnits(uint256(1050e18)), toNAVUnits(uint256(150e18)), ZERO_NAV_UNITS, true);
    ...
    function test_PostOp_coverageGate_jtRedeemExactBoundary() public {
        ...
        SyncedAccountingState memory state = kernel.doPostOp(Operation.JT_REDEEM, toNAVUnits(uint256(1000e18)), toNAVUnits(SEED_LPT_RAW), ZERO_NAV_UNITS, true);
        assertEq(state.coverageUtilizationWAD, WAD, "coverage utilization lands exactly on WAD and passes");
        vm.expectRevert(IRoycoDayAccountant.COVERAGE_REQUIREMENT_VIOLATED.selector);
        kernel.doPostOp(Operation.JT_REDEEM, toNAVUnits(uint256(1000e18 - 1)), toNAVUnits(SEED_LPT_RAW), ZERO_NAV_UNITS, true);
    }
    ...
    /**
     * the liquidity gate for LPT_MULTI_ASSET_DEPOSIT passes at exactly WAD and fires at WAD + 1
     * Arithmetic: minting senior to stEffectiveNAV 2020e18 against lptRawNAV 101e18 gives liquidityUtilization = ceil(2020e18 * 0.05e18
    ...
    function test_PostOp_liquidityGate_lptMultiAssetDepositExactBoundary() public {
        _seedState(SEED_ST_EFF, 300e18, 0, 100e18, MarketState.PERPETUAL);
        SyncedAccountingState memory state =
            kernel.doPostOp(Operation.LPT_MULTI_ASSET_DEPOSIT, toNAVUnits(uint256(2320e18)), toNAVUnits(uint256(101e18)), ZERO_NAV_UNITS, true);
        assertEq(state.liquidityUtilizationWAD, WAD, "liquidity utilization lands exactly on WAD and passes");
        vm.expectRevert(IRoycoDayAccountant.LIQUIDITY_REQUIREMENT_VIOLATED.selector);
        kernel.doPostOp(Operation.LPT_MULTI_ASSET_DEPOSIT, toNAVUnits(uint256(2320e18 + 21)), toNAVUnits(uint256(101e18 + 1)), ZERO_NAV_UNITS, true);
    }

    /**
     * the liquidity gate for LPT_REDEEM passes at exactly WAD and fires at WAD + 1
    ...
    /**
     * the liquidity gate for LPT_MULTI_ASSET_REDEEM passes at exactly WAD and fires at WAD + 1
     * Arithmetic: the redemption unwinds 100e18 of senior alongside the BPT slice, so the boundary sits at
     * lptRawNAV 45e18: liquidityUtilization = ceil(900e18 * 0.05e18 / 45e18) = 1e18 exactly, one more redeemed
     * BPT wei gives ceil(4.5e37 / (4.5e19 - 1)) = 1e18 + 1
     */
    function test_PostOp_liquidityGate_lptMultiAssetRedeemExactBoundary() public {
        _seedFlatWithLPT(SEED_LPT_RAW);
        SyncedAccountingState memory state =
            kernel.doPostOp(Operation.LPT_MULTI_ASSET_REDEEM, toNAVUnits(SEED_COLLATERAL - 100e18), toNAVUnits(uint256(45e18)), ZERO_NAV_UNITS, true);
        assertEq(state.liquidityUtilizationWAD, WAD, "liquidity utilization lands exactly on WAD and passes");
        vm.expectRevert(IRoycoDayAccountant.LIQUIDITY_REQUIREMENT_VIOLATED.selector);
        kernel.doPostOp(Operation.LPT_MULTI_ASSET_REDEEM, toNAVUnits(SEED_COLLATERAL - 100e18), toNAVUnits(uint256(45e18 - 1)), ZERO_NAV_UNITS, true);
    }

    /**
     * an in-kind BPT-only LPT deposit that IMPROVES a breached liquidity utilization passes under enforcement
    ...
    function test_PostOp_liquidityGate_lptDepositHealingPassesUnderBreach() public {
        ...
        SyncedAccountingState memory state =
            kernel.doPostOp(Operation.LPT_DEPOSIT, toNAVUnits(SEED_COLLATERAL), toNAVUnits(uint256(25e18)), ZERO_NAV_UNITS, true);
        ...
        state = kernel.doPostOp(Operation.LPT_DEPOSIT, toNAVUnits(SEED_COLLATERAL), toNAVUnits(uint256(50e18)), ZERO_NAV_UNITS, true);
    ...
    /// LPT_REDEEM passes an enforced coverage breach because a liquidity redemption cannot add senior exposure
    function test_PostOp_gateExemptions_lptRedeemPassesCoverageBreach() public {
        _seedState(SEED_ST_EFF, 50e18, 0, SEED_LPT_RAW, MarketState.PERPETUAL);
        SyncedAccountingState memory state =
            kernel.doPostOp(Operation.LPT_REDEEM, toNAVUnits(uint256(1050e18)), toNAVUnits(uint256(60e18)), ZERO_NAV_UNITS, true);
    ...
    /**
     * LPT_MULTI_ASSET_REDEEM passes an enforced coverage breach because unwinding senior exposure can only
     * improve coverage
    ...
    function test_PostOp_gateExemptions_lptMultiAssetRedeemPassesCoverageBreach() public {
        _seedState(SEED_ST_EFF, 50e18, 0, SEED_LPT_RAW, MarketState.PERPETUAL);
        SyncedAccountingState memory state =
            kernel.doPostOp(Operation.LPT_MULTI_ASSET_REDEEM, toNAVUnits(uint256(1040e18)), toNAVUnits(uint256(60e18)), ZERO_NAV_UNITS, true);
    ...
    /**
     * Adversarial sequencing: commitLiquidityProviderTrancheRawNAV itself enforces NO gate: a kernel commit can park
    ...
    function test_Commit_isUngatedAndArmsBothGatesForLaterOperations() public {
        ...
        vm.expectRevert(IRoycoDayAccountant.LIQUIDITY_REQUIREMENT_VIOLATED.selector);
        kernel.doPostOp(Operation.LPT_REDEEM, toNAVUnits(SEED_COLLATERAL), toNAVUnits(uint256(10e18 - 1)), ZERO_NAV_UNITS, true);
    }
}
