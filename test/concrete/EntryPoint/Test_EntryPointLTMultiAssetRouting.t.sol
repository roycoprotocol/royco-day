 ...
 */
contract Test_EntryPointLTMultiAssetRouting is EntryPointTestBase {
    ...
    function setUp() public {
        ...
        _deployEntryPoint();
        // Hand the seeded LT position to USER_A so an escrowed redemption can exceed the in-kind bound
        // (the balance read is hoisted: an inline call in the argument list would consume the prank)
        uint256 seededShares = liquidityTranche.balanceOf(LT_PROVIDER);
        vm.prank(LT_PROVIDER);
        liquidityTranche.transfer(USER_A, seededShares);
    }

    /// @dev Queues USER_A's whole LT position for redemption to USER_B and returns the nonce with both post-escrow bounds
    function _requestWholePosition(uint64 _executorBonusWAD) internal returns (uint256 nonce, uint256 maxInKindShares, uint256 maxMultiAssetShares) {
        (nonce,) = _requestRedemption(USER_A, address(liquidityTranche), liquidityTranche.balanceOf(USER_A), USER_B, _executorBonusWAD);
        _warpPastRedemptionDelay();
        maxInKindShares = liquidityTranche.maxRedeem(address(entryPoint));
        maxMultiAssetShares = liquidityTranche.maxRedeemMultiAsset(address(entryPoint));
    ...
    function test_ltRedemption_maxSentinel_fallsBackToMultiAssetBeyondInKindBound() public {
        ...
        (AssetClaims memory previewClaims, uint256 previewQuote) = liquidityTranche.previewRedeemMultiAsset(maxMultiAssetShares);
        ...
        assertEq(toUint256(claims.ltAssets), 0, "a multi-asset exit must carry no LP-token leg");
    ...
    function test_ltRedemption_withinInKindBound_staysInKind() public {
        uint256 shares = 1000e18;
        (uint256 nonce,) = _requestRedemption(USER_A, address(liquidityTranche), shares, USER_B, 0);
        _warpPastRedemptionDelay();
        require(shares <= liquidityTranche.maxRedeem(address(entryPoint)), "setup: the amount must fit the in-kind bound");

        (AssetClaims memory claims, uint256 quoteAssets) = _executeRedemptionMaxWithQuote(USER_A, USER_A, nonce);

        assertGt(toUint256(claims.ltAssets), 0, "an in-kind exit must pay the LP-token leg");
        assertEq(quoteAssets, 0, "an in-kind exit must carry no quote leg");
        assertEq(bpt.balanceOf(USER_B), toUint256(claims.ltAssets), "the LP-token leg must land on the receiver");
        assertEq(quoteToken.balanceOf(USER_B), 0, "no quote may land on an in-kind exit");
    }

    /// @notice Equal bounds resolve in-kind: with no senior-share value in the removal the two bounds coincide,
    ///         and a maximal redemption executes the in-kind bound as a partial in-kind fill
    function test_ltRedemption_equalBounds_defaultsInKind() public {
        ...
        require(liquidityTranche.maxRedeemMultiAsset(address(entryPoint)) == maxInKindShares, "setup: the bounds must coincide");
        ...
        assertGt(toUint256(claims.ltAssets), 0, "the in-kind exit must pay the LP-token leg");
        assertEq(bpt.balanceOf(USER_B), toUint256(claims.ltAssets), "the partial in-kind fill must land on the receiver");
    }
    ...
    function test_ltRedemption_explicitAmounts_alwaysExitInKind() public {
        (uint256 nonce, uint256 maxInKindShares, uint256 maxMultiAssetShares) = _requestWholePosition(0);
        ...
        assertGt(toUint256(inKindClaims.ltAssets), 0, "the in-kind exit must pay the LP-token leg");
    }
    ...
    function test_ltRedemption_maxSentinel_clampsDominantBoundToTheRequest() public {
        // Queue a slice that the in-kind bound cannot serve but the multi-asset bound over-serves
        uint256 maxInKindShares = liquidityTranche.maxRedeem(USER_A);
        uint256 maxMultiAssetShares = liquidityTranche.maxRedeemMultiAsset(USER_A);
        ...
        (uint256 nonce,) = _requestRedemption(USER_A, address(liquidityTranche), requestedShares, USER_B, 0);
        _warpPastRedemptionDelay();
    ...
    function test_ltRedemption_explicitBeyondMultiAssetBound_reverts() public {
    ...
    function test_ltRedemption_thirdParty_multiAsset_splitsQuoteAndConstituentLegs() public {
    ...
    function test_ltRedemption_multiAsset_forfeitsQueuedYieldAsLTShares() public {
        ...
        assertGt(
            ...
            0,
            "queued yield must be forfeited as LT protocol fee shares on the multi-asset route"
        );
    }
    ...
    function test_ltRedemption_maxSentinel_zeroCapacityExecutesNothing() public {
        (uint256 nonce,,) = _requestWholePosition(0);
        uint256 escrowedShares = entryPoint.getRedemptionRequest(USER_A, nonce).shares;
        ...
        _sync();
        require(liquidityTranche.maxRedeemMultiAsset(address(entryPoint)) == 0, "setup: the wiped mark must zero the multi-asset bound");
    ...
    /// @notice ST and JT redemptions never carry a quote leg: the maximizer and the quote lane are liquidity-tranche-only
    ...
    function test_ltRedemption_multiAsset_redemptionExecutedEventCarriesAllLanes() public {
        (uint256 nonce,, uint256 maxMultiAssetShares) = _requestWholePosition(0);
        (AssetClaims memory previewClaims, uint256 previewQuote) = liquidityTranche.previewRedeemMultiAsset(maxMultiAssetShares);
    ...
    /// @notice A batch execution across tranches carries a per-request quote lane: the LT request's multi-asset
    ...
    function test_executeRedemptions_mixedTranches_carryPerRequestQuoteLanes() public {
        (uint256 ltNonce,,) = _requestWholePosition(0);
        ...
        assertGt(quoteAssets[0], 0, "the LT request's multi-asset exit must fill its quote slot");
        assertEq(quoteAssets[1], 0, "the senior request's quote slot must stay zero");
        assertGt(toUint256(claims[0].nav), 0, "the LT request must settle claims");
    ...
    function test_ltRedemption_multiAsset_yieldNeutralByMagnitude() public {
        (uint256 nonce,,) = _requestWholePosition(0);
    ...
    function test_ltRedemption_mixedRoutePartial_conservesSnapshotAndCancellation() public {
        ...
        uint256 balanceBefore = liquidityTranche.balanceOf(USER_A);
        vm.prank(USER_A);
        entryPoint.cancelRedemptionRequest(nonce, USER_A);
        assertEq(liquidityTranche.balanceOf(USER_A) - balanceBefore, sharesLeft, "cancellation must return exactly the unfilled escrow");
    }
    ...
    function test_ltRedemption_maxSentinel_pausedKernelExecutesNothing() public {
        (uint256 nonce,,) = _requestWholePosition(0);
        uint256 escrowedShares = entryPoint.getRedemptionRequest(USER_A, nonce).shares;
    ...
    function test_ltRedemption_zeroNavSnapshotRemainder_fullyForfeitsWithoutReverting() public {
        ...
        applyLTPnL(1000);
        _sync();
        uint256 feeSharesBefore = entryPoint.getProtocolFeeSharesPendingCollection(address(liquidityTranche));
        ...
        assertEq(entryPoint.getProtocolFeeSharesPendingCollection(address(liquidityTranche)) - feeSharesBefore, 1, "the remainder must forfeit whole");
