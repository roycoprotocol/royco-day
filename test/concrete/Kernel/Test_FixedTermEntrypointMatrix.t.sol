 ...
 *      mid-term, everything that only deepens liquidity is. So stDeposit/jtDeposit and an ST-leg multi-asset LPT
 *      deposit revert DISABLED_IN_FIXED_TERM_STATE, while an in-kind BPT LPT deposit and a quote-only multi-asset
 *      LPT deposit succeed.
 */
contract Test_FixedTermEntrypointMatrix is DayMarketTestBase {
    ...
    /// @dev Mints BPT to `_to` backed by a quote-only pool leg (NAV-per-BPT stays ~1.0), for the in-kind LPT deposit.
    ...
    // All tranche deposits are role-gated (restricted); use the seeded role-holders (or grant the LPT role to
    ...
    function test_RevertIf_MultiAssetLPTDepositWithSTLegInFixedTerm() public {
        address a = makeAddr("FT_LPT_STLEG");
        accessManager.grantRole(LPT_LP_ROLE, a, 0);
        stJtVault.mintShares(a, collateralUnit);
        quoteToken.mint(a, 10 * quoteUnit);
        vm.startPrank(a);
        stJtVault.approve(address(liquidityProviderTranche), collateralUnit);
        quoteToken.approve(address(liquidityProviderTranche), 10 * quoteUnit);
        vm.expectRevert(IRoycoDayKernel.DISABLED_IN_FIXED_TERM_STATE.selector);
        liquidityProviderTranche.depositMultiAsset(collateralUnit, 10 * quoteUnit, 0, a);
        vm.stopPrank();
    }
    ...
    function test_QuoteOnlyMultiAssetLPTDeposit_SucceedsInFixedTerm() public {
        address a = makeAddr("FT_LPT_QUOTE");
        accessManager.grantRole(LPT_LP_ROLE, a, 0);
        quoteToken.mint(a, 10 * quoteUnit);
        vm.startPrank(a);
        quoteToken.approve(address(liquidityProviderTranche), 10 * quoteUnit);
        (uint256 shares,) = liquidityProviderTranche.depositMultiAsset(0, 10 * quoteUnit, 0, a);
        vm.stopPrank();
        assertGt(shares, 0, "quote-only multi-asset LPT deposit must mint shares in FIXED_TERM");
    }

    function test_InKindLPTDeposit_SucceedsInFixedTerm() public {
        address a = makeAddr("FT_LPT_INKIND");
        accessManager.grantRole(LPT_LP_ROLE, a, 0);
        uint256 bptAmount = 10e18;
        _mintBptTo(a, bptAmount, 10 * quoteUnit);
        vm.startPrank(a);
        bpt.approve(address(liquidityProviderTranche), bptAmount);
        uint256 shares = liquidityProviderTranche.deposit(toTrancheUnits(bptAmount), a);
        vm.stopPrank();
        assertGt(shares, 0, "in-kind BPT LPT deposit must mint shares in FIXED_TERM");
    }
}
