 ...
 */
contract Test_PreviewSyncLPTClaimsNavParity is DayMarketTestBase {
    function setUp() public {
        _deployMarket(cellA(), defaultParams());
        // ST 100e18 / JT 30e18 vault shares plus the base's auto-seeded quote-only LPT depth
    ...
    function _assertLPTClaimsNavMatchesPostMintPair(string memory _ctx) internal {
        ...
        assertEq(
            toUint256(lptClaims.nav), expectedNav, string.concat(_ctx, ": preview LPT claims.nav must value its post-mint stShares at the post-mint senior pair")
        );
    }

    /**
     * @notice With no pending accrual the parity holds trivially: no premium or fee shares are pending, the
     *         post-mint pair equals the storage pair, and claims.nav == lptRawNAV (+ any staged share value)
     */
    function test_PreviewLPTClaimsNav_MatchesPostMintPair_NoPendingAccrual() public {
        _assertLPTClaimsNavMatchesPostMintPair("flat market");
    }
     ...
     */
    function test_PreviewLPTClaimsNav_MatchesPostMintPair_PendingPremiumAccrual() public {
        ...
        _assertLPTClaimsNavMatchesPostMintPair("pending premium accrual");
    }

    /**
     * @notice The consumer-surface pin: an in-kind LPT previewRedeem quoted across a PENDING premium accrual must
     *         match the claims the actual redeem settles, leg for leg — including the nav leg, which scales the
     *         preview's LPT effective NAV and therefore diverged before the post-mint-pair fix
     * @dev Venue slippage is armed so the redeem's pre-op sync stages the premium instead of deploying it,
     *      keeping the LPT composition (BPT depth + idle senior shares) identical between the preview instant and
    ...
    function test_PreviewRedeem_MatchesRedeem_AcrossPendingPremiumAccrual() public {
        ...
        uint256 shares = liquidityProviderTranche.balanceOf(LPT_PROVIDER) / 20;
        AssetClaims memory quoted = liquidityProviderTranche.previewRedeem(shares);

        vm.prank(LPT_PROVIDER);
        AssetClaims memory settled = liquidityProviderTranche.redeem(shares, LPT_PROVIDER, LPT_PROVIDER);

        assertEq(toUint256(quoted.lptAssets), toUint256(settled.lptAssets), "quoted vs settled: lptAssets (BPT slice)");
