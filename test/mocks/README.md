...
| Pool hooks | Production ships the liquidity-tranche pool with no hook: the accounting hook was removed, so external pool operations need no pre-operation sync | No hook layer at all | Matches production: external swaps/joins do not pre-sync the kernel, so any test simulating third-party pool flow must sync explicitly if it needs fresh committed marks |
