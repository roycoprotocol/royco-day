...
| Swap surface | Full swap surface with E-CLP curve math (price impact, imbalance fees), `onlyWhenUnlocked`, reached through the Router | `swap` mirrors the ACCESS shape (EXACT_IN only, unlock-gated, session debt/credit, `AmountGivenZero` / `CannotSwapSameToken` / `SwapLimit` error shapes, reached through `MockBalancerRouter.swapSingleTokenExactIn`) but prices at the pinned per-token rates: amountOut is the input's fair value less a settable `swapFeeBps` retained by the pool | Exogenous swaps drift pool COMPOSITION and fee value at constant prices, so the two-step mark moves only through TVL; curve-shape realism (a swap moving the mark down via price impact) is fork-suite territory |
