...
/// @notice Mock kernel giving fixed rate accountant tests full control over the onlyRoycoKernel surface
...
contract MockFixedRateAccountantKernel {
    enum SyncMode {
        NONE,
        SYNC,
        REVERT
    ...
    IRoycoDayFixedRateAccountant public accountant;
    ...
    IRoycoDayFixedRateAccountant.RoycoDayFixedRateAccountantState internal _fixedRateStateAtLastSync;
    ...
    function setAccountant(address _accountant) external {
        accountant = IRoycoDayFixedRateAccountant(_accountant);
    }
    ...
    function syncTrancheAccountingFromAccountant() public returns (SyncedAccountingState memory state) {
        ...
        _fixedRateStateAtLastSync = accountant.getRoycoDayFixedRateAccountantState();
        stateHashAtSync.push(keccak256(abi.encode(_stateAtLastSync, _fixedRateStateAtLastSync)));
