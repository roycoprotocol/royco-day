...
/**
 * @title Test_MarketLifecycle_NoReturnValueQuote_NonStandardTokens
 * @notice Market lifecycle on a USDT-shaped quote stable returning EMPTY returndata on its transfer paths,
 *         over the baseline 4626(18,18) ST/JT shares and 6-decimal quote
 * @dev The inherited lifecycle's exact constants hold unchanged: empty returndata only breaks callers that
 *      strictly decode the declared bool, and every quote pull in the seed and venue flows goes through
 *      SafeERC20, which accepts it. This shape's value is proving the exact numbers survive with the USDT
 *      shape ARMED
 ...
 */
contract Test_MarketLifecycle_NoReturnValueQuote_NonStandardTokens is Test_MarketLifecycleBase {
     ...
     *      whole underlying to exactly 1e18 NAV wei. The quote's returndata shape changes no decimals and no rate
     */
     ...
     *      matters: without the probe, a green run on this shape could mean the mock silently ignored the
     *      behavior bitmap. Proving the USDT shape fires, then landing the same wei-exact depth every shape
     *      seeds, shows the venue's quote pulls tolerate a non-bool-returning stable end to end
     */
    function test_NoReturnValueQuote_isLiveAndSafeERC20FlowsStillLandExactly() public {
        ...
        assertEq(returnData.length, 0, "the armed shape must return empty returndata, not the declared bool");
