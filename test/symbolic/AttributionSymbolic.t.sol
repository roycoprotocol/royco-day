...
/**
 * @title AttributionHarness
 * @notice Exposes the accountant's internal signed PnL attribution helper for symbolic execution. The helper is
 *         pure, so the harness deploys with a dummy kernel address and never touches accountant state
 */
contract AttributionHarness is RoycoDayAccountant {
...
/**
 * @title AttributionSymbolic
 * @notice Halmos symbolic specs for the signed PnL attribution step of the sync waterfall. The load-bearing
 *         properties: a zero delta, claim, or checkpoint NAV attributes nothing, every other attribution is the
 *         exactly floored pro-rata slice of the delta with the delta's sign preserved and magnitude never
 *         exceeding the delta, and splitting one claim into two loses at most one wei to flooring in the
 *         direction that shorts the claimant (the complementary tranche absorbs the dust)
...
contract AttributionSymbolicSpec is Test {
    ...
    AttributionHarness internal harness;

    function setUp() public {
        harness = new AttributionHarness();
    }
    ...
    function check_zeroDeltaClaimOrCheckpointAttributesNothing(int256 delta, uint256 claim, uint256 lastRaw) external view {
        ...
        int256 attributed = harness.attributeDelta(delta, toNAVUnits(claim), toNAVUnits(lastRaw));
    ...
    function check_attributionIsTheFlooredProRataSliceWithSignPreserved(int256 delta, uint256 claim, uint256 lastRaw) external view {
        // A live checkpoint with a claim carved out of it: 0 < claim <= lastRaw
    ...
    function check_splittingAClaimLosesAtMostOneWeiAgainstTheSplitSide(int256 delta, uint256 claimA, uint256 claimB, uint256 lastRaw) external view {
        // Two disjoint claims carved out of one live checkpoint
        vm.assume(delta >= -int256(MAX_NAV) && delta <= int256(MAX_NAV) && delta != 0);
        vm.assume(1 <= lastRaw && lastRaw <= MAX_NAV);
        vm.assume(claimA <= lastRaw && claimB <= lastRaw - claimA);

        int256 whole = harness.attributeDelta(delta, toNAVUnits(claimA + claimB), toNAVUnits(lastRaw));
        int256 splitSum =
            harness.attributeDelta(delta, toNAVUnits(claimA), toNAVUnits(lastRaw)) + harness.attributeDelta(delta, toNAVUnits(claimB), toNAVUnits(lastRaw));
