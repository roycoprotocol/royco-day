...
contract RoycoLiquidityTranche is RoycoVaultTranche, IRoycoLiquidityTranche {
    ...
    constructor(
        ...
        RoycoVaultTranche(_asset, _kernel, _enforceVaultSharesTransferWhitelist, _quoter, _hook)
    { }
    ...
    function previewDepositMultiAsset(uint256 _stAssets, uint256 _quoteAssets) external virtual override(IRoycoLiquidityTranche) returns (uint256 shares) {
        ...
        (NAV_UNIT valueAllocated, NAV_UNIT navToMintSharesAt,) = IRoycoDayQuoter(QUOTER).ltPreviewDepositMultiAsset(toTrancheUnits(_stAssets), _quoteAssets);
        ...
        returns (AssetClaims memory stClaims, uint256 quoteAssets)
    {
        ...
        (stClaims, quoteAssets) = IRoycoDayQuoter(QUOTER).ltPreviewRedeemMultiAsset(_shares);
    }
}
