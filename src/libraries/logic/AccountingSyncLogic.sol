...
library AccountingSyncLogic {
        ...
        returns (SyncedAccountingState memory state, AssetClaims memory claims, uint256 totalTrancheShares)
    {
        ...
        if (_trancheType == TrancheType.SENIOR) {
            ...
            (,, totalTrancheShares) = FeeAndLiquidityPremiumLogic._computeSTFeeAndLiquidityPremiumSharesToMint(state, IERC20($.seniorTranche).totalSupply());
        } else if (_trancheType == TrancheType.JUNIOR) {
            // Compute JT share supply after the JT protocol fee shares are minted
            totalTrancheShares = IERC20($.juniorTranche).totalSupply();
            ...
            (uint256 liquidityPremiumShares,, uint256 stTotalSupplyAfterMints) =
                FeeAndLiquidityPremiumLogic._computeSTFeeAndLiquidityPremiumSharesToMint(state, IERC20($.seniorTranche).totalSupply());
            ...
            totalTrancheShares = IERC20($.liquidityProviderTranche).totalSupply();
        }
    }
        ...
        returns (SyncedAccountingState memory state)
    {
        ...
        uint256 totalSTShares = IERC20($.seniorTranche).totalSupply();
    ...
    function _commitLPTRawNAV(IRoycoDayKernel.RoycoDayKernelState storage $, SyncedAccountingState memory _state) internal {
        ...
        IRoycoDayAccountant($.accountant).commitLiquidityProviderTrancheRawNAV(lptRawNAV);
