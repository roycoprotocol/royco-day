...
interface IRoycoLiquidityTranche is IRoycoVaultTranche {
    /**
     * @notice Emitted on a multi-asset LT deposit (collateral + quote -> LP token -> LT shares)
    ...
    event MultiAssetDeposit(
        address indexed caller, address indexed receiver, uint256 collateralAssets, uint256 quoteAssets, uint256 ltAssetsMinted, uint256 shares
    );

    /**
     * @notice Emitted on a multi-asset LT redemption (LT shares -> LP token -> collateral + quote)
    ...
    /**
     * @notice Enters the LT with the LP token's constituent assets: collateral + quote
    ...
    /**
     * @notice Exits the LT to the LP token's constituent assets: collateral + quote
     * @dev The kernel proportionally removes the LP-token slice, redeems the pooled senior shares to collateral, and
     *      transfers the collateral and quote directly to the receiver, the LT shares are burned afterwards
    ...
    /**
     * @notice Previews a multi-asset LT deposit of (collateral + quote): the LT shares it would mint and the LT tranche assets the add would produce
     ...
     */
    function previewDepositMultiAsset(uint256 _collateralAssets, uint256 _quoteAssets) external returns (uint256 shares, uint256 ltAssetsOut);

    /**
     * @notice Previews a multi-asset LT redemption of _shares: the collateral claims and quote it would yield
