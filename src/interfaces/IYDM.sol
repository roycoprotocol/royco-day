 ...
 * @notice Interface for yield distribution models that price the premium paid to a pool of capital for a service it provides to the market
 * @dev A YDM is a general-purpose premium model: it maps a single utilization input to the share of a discrete amount of distributed yield paid out to the capital pool as a premium
 ...
 * @dev The accountant defines the yield being distributed (eg. the senior gain or the excess yield above a coupon), the model prices only the share, so it is agnostic to the source of the yield it splits
 */
interface IYDM {
    ...
    /**
     * @notice Previews and returns the share of the distributed yield to pay the capital pool as a premium at the given utilization
    ...
    /**
     * @notice Returns the share of the distributed yield to pay the capital pool as a premium at the given utilization
