...
**ST Losses**: JT provides coverage from its remaining buffer (its effective NAV) up to the full loss. The coverage provided is tracked as the JT coverage impermanent loss, JT's first claim on future ST appreciation. Any ST loss exceeding JT's remaining buffer is borne directly by senior effective NAV, and a market whose junior buffer is exhausted against live senior exposure is uncollateralized, which forces the perpetual state and resets JT's recovery claim.

**JT Losses**: Reduce JT effective NAV directly and never spill into senior effective NAV. Senior's exposure to junior-held assets exists only through the claims created by previously applied coverage, and those settle proportionally in the PnL attribution rather than as spillover.

**Recovery**: When ST appreciates, the JT coverage impermanent loss has first claim on the gains and is repaid to junior. Residual gains are distributed as yield via the two YDM instances, the risk premium to junior and the liquidity premium to the liquidity tranche, protocol fees accrue, and the remainder accrues to senior.
