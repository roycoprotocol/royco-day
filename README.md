...
A template encodes one market recipe and atomically deploys the full market at deterministic addresses, wiring every component's roles on the shared authority. This covers the senior, junior, and liquidity provider tranches, the kernel and accountant, and the liquidity provider tranche's market-making venue and oracle. The risk and liquidity premium yield models are user-supplied deployment parameters like the collateral oracle: each market passes its model addresses at deployment and initializes its curves on them, and one instance can serve both premiums since curves are keyed per tranche type. Each template pins the market's component implementations through beacons and construction immutables, so every market it deploys runs the same audited implementation set. The template also carries the governance-set protocol fee and venue policy that every future market is deployed with.
...
5. Distributing yield from senior to the liquidity provider tranche (liquidity premium) as instructed by the liquidity-driven YDM curve
...
**Recovery**: When the collateral appreciates, the JT impermanent loss is repaid off the top of the gain before any distribution, restoring junior's claim at its original proportions so a dip-and-recover path lands exactly where the direct path would. The residual gain splits pro-rata across the restored claims: junior's share is junior yield, and senior's share pays the risk premium to junior and the liquidity premium to the liquidity provider tranche via the two YDM curves, accrues protocol fees, and the remainder accrues to senior. Repayment is restoration, never yield, so it is never fee'd.
