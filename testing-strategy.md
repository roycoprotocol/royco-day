...
| F9 | Shares: `⌊supply·value/totalValue⌋`; supply=0 ⇒ `value` 1:1; totalValue=0 ⇒ denominator **1 wei** | `ValuationLogic.sol:102-107` | Floor | existing holders |
...
| I17 | Boundary semantics: first deposit mints `value` shares 1:1; `totalValue==0 ∧ supply>0` mint uses 1-wei denominator (F9) — behavior pinned by unit vectors and bounded in handler (no overflow, no free value: post-state still satisfies I1/I2) | U,H | |
...
4. **`_convertToShares` zero-NAV branch uses a 1-wei denominator** (`ValuationLogic.sol:106`): a deposit into a tranche with live supply and zero NAV mints `supply × value` shares. Commented as intentional dilution of unbacked holders; the magnitude (potential ~1e48 supplies) deserves an explicit overflow/griefing review. Pinned by I17 either way.
