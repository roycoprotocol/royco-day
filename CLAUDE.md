...
- The Balancer E-CLP oracle adapter (`src/oracles/venues/balancer-v3/*`): reads Balancer's native E-CLP oracle for `ltRawNAV`, plus the rate provider. No valuation math is reimplemented. The pool is registered with no hook: an external pool operation needs no pre-operation sync, because the senior rate comes from the kernel's `getRate` cache-miss preview and the other reads use the oracle and the committed checkpoint. The abstraction lets any AMM or MM vault back the LT; Balancer E-CLP is first.
...
- P4, Balancer oracle. Read `ltRawNAV` from Balancer's E-CLP oracle. Wire the rate provider (ST share NAV from the last committed sync). The pool registers no hook: an external operation needs no pre-operation sync for any Day quantity to be correct.
